...
2) On top of this we would like to get to functionally test WCA with real customers that are somehow lower priority before rolling out changes to all customers everywhere.
## The Solution

To solve both these problems, we create "deployment environments" known simply as "environments". Customer WCA is deployed to an environment known as "PROD" (short for production). PROD is a bit of an abstract concept, but it is any WCA related code/config/infrastructure/AD related to running WCA for our paid CW customers. For example, we call the code that runs WCA for customers "PROD WCA". The backend for PROD WCA runs on dedicated servers. These are PROD servers.
