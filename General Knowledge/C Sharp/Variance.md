...
Variance needs two things to exist, without these it doesn't make sense to talk about variance. The first is that the set of types in a programming language need to form a poset. This is the case in C#. Let $P$ be the set of all types. There is a partial order $(P, \le)$ where $\le \subseteq P \times P$ given by $a \le b$ if an instance of $b$ can be assigned to an identifier of type 
