...
Answer:
- The .NET Thread Pool  handles returning threads and allocating threads for queued work
- The TaskScheduler (default for thread pool) decides which thread executes the continuation
- The state machine generated by async/await tells the runtime what code to run next