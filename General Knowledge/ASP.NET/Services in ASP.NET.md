...
Since services have clear concerns, any work that needs to be done that requires data or logic that the service does not have access to is handled by 'dependencies'. An instance of the class that does have the power/permission/data to execute an operation (typically another service) is provided to the service class as an object through the constructor. This object is known as a dependency, as the primary service depends on it to function. The primary service now has the ability to invoke specific functionalities through the specialized dependency.
