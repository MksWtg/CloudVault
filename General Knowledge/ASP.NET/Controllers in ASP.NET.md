...
The above code is an example controller. It is a class, it has a rule for what requests come to it based on URL (those ending with `/api/hello`). Controllers have methods for different subroutines that should run based on the path parameters of the request (e.g. a request ending in `items` might go to one C# method and requests ending in `buy` might go to another).

This controller has one method `Get` that will only be accessed if the incoming HTTP request uses a `Get` HTTP verb. It returns an `IActionResult` object with a message. `Ok` returns code `200`.