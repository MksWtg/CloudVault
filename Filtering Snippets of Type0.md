...
Algebra\LindenbaumTarski.agda:40:data Formula : Type where
Algebra\LindenbaumTarski.agda:49:data ctxt : Type where
Algebra\LindenbaumTarski.agda:54:data _∈_ : Formula → ctxt → Type where
Algebra\LindenbaumTarski.agda:61:data _⊢_ : ctxt → Formula → Type where
Algebra\LindenbaumTarski.agda:147:  _∼_ : Formula → Formula → Type
Algebra\LindenbaumTarski.agda:274:  LindenbaumTarski : Type
Algebra\Matrix.agda:36:    A : Type ℓ
Algebra\Matrix.agda:40:FinMatrix : (A : Type ℓ) (m n : ℕ) → Type ℓ
Algebra\Matrix.agda:43:VecMatrix : (A : Type ℓ) (m n : ℕ) → Type ℓ
Algebra\Matrix.agda:70:FinMatrixIsoVecMatrix : (A : Type ℓ) (m n : ℕ) → Iso (FinMatrix A m n) (VecMatrix A m n)
Algebra\Matrix.agda:79:FinMatrix≡VecMatrix : (A : Type ℓ) (m n : ℕ) → FinMatrix A m n ≡ VecMatrix A m n
Algebra\Matrix.agda:280:flattenElim : {P : A → Type ℓ'} {n m : ℕ} (M : FinMatrix A n m)
Algebra\SymmetricGroup.agda:26:  X Y : Type ℓ
Algebra\SymmetricGroup.agda:28:SymGroup : (X : Type ℓ) → isSet X → Group ℓ
Algebra\AbGroup\Base.agda:35:record IsAbGroup {A : Type ℓ}
Algebra\AbGroup\Base.agda:36:                 (0g : A) (_+_ : A → A → A) (-_ : A → A) : Type ℓ where
Algebra\AbGroup\Base.agda:60:record AbGroupStr (A : Type ℓ) : Type (ℓ-suc ℓ) where
Algebra\AbGroup\Base.agda:76:AbGroup : ∀ ℓ → Type (ℓ-suc ℓ)
Algebra\AbGroup\Base.agda:77:AbGroup ℓ = TypeWithStr ℓ AbGroupStr
Algebra\AbGroup\Base.agda:79:module _ {G : Type ℓ} {0g : G} {_+_ : G → G → G} { -_ : G → G}
Algebra\AbGroup\Base.agda:95:module _ {G : Type ℓ} (0g : G) (_+_ : G → G → G) (-_ : G → G)
Algebra\AbGroup\Base.agda:114:AbGroupStr→GroupStr : {G : Type ℓ} → AbGroupStr G → GroupStr G
Algebra\AbGroup\Base.agda:140:AbGroupHom : (G : AbGroup ℓ) (H : AbGroup ℓ') → Type (ℓ-max ℓ ℓ')
Algebra\AbGroup\Base.agda:143:AbGroupIso : (G : AbGroup ℓ) (H : AbGroup ℓ') → Type (ℓ-max ℓ ℓ')
Algebra\AbGroup\Base.agda:146:IsAbGroupEquiv : {A : Type ℓ} {B : Type ℓ'}
Algebra\AbGroup\Base.agda:147:  (G : AbGroupStr A) (e : A ≃ B) (H : AbGroupStr B) → Type (ℓ-max ℓ ℓ')
Algebra\AbGroup\Base.agda:150:AbGroupEquiv : (G : AbGroup ℓ) (H : AbGroup ℓ') → Type (ℓ-max ℓ ℓ')
Algebra\AbGroup\Base.agda:153:isPropIsAbGroup : {G : Type ℓ} (0g : G) (_+_ : G → G → G) (-_ : G → G)
Algebra\AbGroup\Base.agda:181:-- equivalence between an abelian group G and a type A which preserves
Algebra\AbGroup\Base.agda:183:-- when proving that some type equivalent to an abelian group is an
Algebra\AbGroup\Base.agda:186:module _ (G : AbGroup ℓ) {A : Type ℓ}
Algebra\AbGroup\Base.agda:199:    BaseΣ : Type (ℓ-suc ℓ)
Algebra\AbGroup\Base.agda:200:    BaseΣ = Σ[ B ∈ Type ℓ ] (B → B → B) × B × (B → B)
Algebra\AbGroup\Base.agda:202:    FamilyΣ : BaseΣ → Type ℓ
Algebra\AbGroup\Base.agda:229:-- isomorphism). This version is useful when proving that some type
Algebra\AbGroup\Base.agda:234:module _ (G : AbGroup ℓ) {A : Type ℓ}
Algebra\AbGroup\Base.agda:243:    FamilyΣ : Σ[ B ∈ Type ℓ ] (B → B → B) → Type ℓ
Algebra\AbGroup\Base.agda:249:        (UARel.≅→≡ (autoUARel (Σ[ B ∈ Type ℓ ] (B → B → B))) (e , p·))
Algebra\AbGroup\Base.agda:287:move4 : ∀ {ℓ} {A : Type ℓ} (x y z w : A) (_+_ : A → A → A)
Algebra\AbGroup\Base.agda:296:---- The type of homomorphisms A → B is an AbGroup if B is -----
Algebra\AbGroup\FinitePresentation.agda:23:record FinitePresentation (A : AbGroup ℓ) : Type ℓ where
Algebra\AbGroup\FinitePresentation.agda:30:isFinitelyPresented : AbGroup ℓ → Type ℓ
Algebra\AbGroup\TensorProduct.agda:51:  data _⨂₁_ : Type (ℓ-max ℓ ℓ') where
Algebra\AbGroup\TensorProduct.agda:85:  ⊗elimProp : ∀ {ℓ} {C : AGr ⨂₁ BGr → Type ℓ}
Algebra\AbGroup\TensorProduct.agda:157:  ⊗elimPropUnlist : ∀ {ℓ} {C : AGr ⨂₁ BGr → Type ℓ}
Algebra\AbGroup\TensorProduct.agda:304:isTensorProductOf_and_ : AbGroup ℓ → AbGroup ℓ' → AbGroup (ℓ-max ℓ ℓ')→ Type _
Algebra\AbGroup\Instances\DirectSumFun.agda:14:module _ (G : (n : ℕ) → Type ℓ) (Gstr : (n : ℕ) → AbGroupStr (G n)) where
Algebra\AbGroup\Instances\DirectSumHIT.agda:13:module _ (Idx : Type ℓ) (P : Idx → Type ℓ') (AGP : (r : Idx) → AbGroupStr (P r)) where
Algebra\AbGroup\Instances\FreeAbGroup.agda:38:module _ {A : Type ℓ} where
Algebra\AbGroup\Instances\FreeAbGroup.agda:43:FAGAbGroup→AbGroupHom : ∀ {ℓ ℓ'} {A : Type ℓ} {G : AbGroup ℓ'}
Algebra\AbGroup\Instances\FreeAbGroup.agda:52:FAGAbGroupGroupHom≡ : ∀ {ℓ ℓ'} {A : Type ℓ} {G : AbGroup ℓ'}
Algebra\AbGroup\Instances\FreeAbGroup.agda:65:module _ {A : Type ℓ} where
Algebra\AbGroup\Instances\FreeAbGroup.agda:186:  → (P : ℤ[Fin (suc n) ] .fst → Type ℓ)
Algebra\AbGroup\Instances\FreeAbGroup.agda:196:  module _ {ℓ} (n : ℕ) (P : ℤ[Fin (suc n) ] .fst → Type ℓ)
Algebra\AbGroup\Instances\FreeAbGroup.agda:399:  (A : ℤ[Fin n ] .fst → Type ℓ)
Algebra\AbGroup\Instances\NProd.agda:16:NProd-AbGroup : (G : (n : ℕ) → Type ℓ) → (Gstr : (n : ℕ) → AbGroupStr (G n)) → AbGroup ℓ
Algebra\AbGroup\Instances\Pi.agda:9:module _ {ℓ ℓ' : Level} {X : Type ℓ} (G : X → AbGroup ℓ') where
Algebra\AbGroup\Instances\Pi.agda:14:ΠℤAbGroup : ∀ {ℓ} (A : Type ℓ) → AbGroup ℓ
Algebra\AbGroup\Instances\Unit.agda:6:open import Cubical.Data.Unit renaming (Unit* to UnitType)
Algebra\AbGroup\Instances\Unit.agda:20:fst UnitAbGroup = UnitType
Algebra\Algebra\Base.agda:29:record IsAlgebra (R : Ring ℓ) {A : Type ℓ'}
Algebra\Algebra\Base.agda:31:                 (_⋆_ : ⟨ R ⟩ → A → A) : Type (ℓ-max ℓ ℓ') where
Algebra\Algebra\Base.agda:54:record AlgebraStr (R : Ring ℓ) (A : Type ℓ') : Type (ℓ-max ℓ ℓ') where
Algebra\Algebra\Base.agda:69:Algebra : (R : Ring ℓ) → ∀ ℓ' → Type (ℓ-max ℓ (ℓ-suc ℓ'))
Algebra\Algebra\Base.agda:70:Algebra R ℓ' = Σ[ A ∈ Type ℓ' ] AlgebraStr R A
Algebra\Algebra\Base.agda:110:  module _ {A : Type ℓ'} {0a 1a : A}
Algebra\Algebra\Base.agda:142:record IsAlgebraHom {R : Ring ℓ} {A : Type ℓ'} {B : Type ℓ''}
Algebra\Algebra\Base.agda:144:  : Type (ℓ-max ℓ (ℓ-max ℓ' ℓ''))
Algebra\Algebra\Base.agda:168:AlgebraHom : (M : Algebra R ℓ') (N : Algebra R ℓ'') → Type _
Algebra\Algebra\Base.agda:171:IsAlgebraEquiv : {A : Type ℓ'} {B : Type ℓ''}
Algebra\Algebra\Base.agda:173:  → Type _
Algebra\Algebra\Base.agda:176:AlgebraEquiv : (M : Algebra R ℓ') (N : Algebra R ℓ'') → Type _
Algebra\Algebra\Base.agda:185:isPropIsAlgebra : (R : Ring ℓ) {A : Type ℓ'}
Algebra\Algebra\Base.agda:202:isPropIsAlgebraHom : (R : Ring ℓ) {A : Type ℓ'} {B : Type ℓ''}
Algebra\Algebra\Properties.agda:168:      isoOnTypes : Iso (fst B → fst C) (fst A → fst C)
Algebra\Algebra\Properties.agda:169:      isoOnTypes = equivToIso (_ , (snd (preCompEquiv (fst f))))
Algebra\Algebra\Properties.agda:180:          (isoOnTypes .sec (h .fst))
Algebra\Algebra\Properties.agda:184:          (isoOnTypes .ret (g .fst))
Algebra\Algebra\Properties.agda:186:isSetAlgebraStr : (A : Type ℓ') → isSet (AlgebraStr R A)
Algebra\Algebra\Subalgebra.agda:21:record isSubalgebra (S : ℙ ⟨ A ⟩) : Type (ℓ-max ℓ ℓ') where
Algebra\Algebra\Subalgebra.agda:50:Subalgebra : Type (ℓ-max ℓ (ℓ-suc ℓ'))
Algebra\BooleanRing\Base.agda:16:record IsBooleanRing {B : Type ℓ}
Algebra\BooleanRing\Base.agda:17:  (𝟘 𝟙 : B) (_+_ _·_ : B → B → B) (-_ : B → B) : Type ℓ where
Algebra\BooleanRing\Base.agda:26:record BooleanRingStr (A : Type ℓ) : Type (ℓ-suc ℓ) where
Algebra\BooleanRing\Base.agda:41:BooleanRing : ∀ ℓ → Type (ℓ-suc ℓ)
Algebra\BooleanRing\Base.agda:42:BooleanRing ℓ = TypeWithStr ℓ BooleanRingStr
Algebra\BooleanRing\Base.agda:44:module _ {A : Type ℓ} (BRStr : BooleanRingStr A) where
Algebra\BooleanRing\Base.agda:58:BooleanRingStr→RingStr : { A : Type ℓ } → BooleanRingStr A → RingStr A
Algebra\BooleanRing\Base.agda:64:isIdemRing : {ℓ : Level} → CommRing ℓ → Type ℓ
Algebra\BooleanRing\Base.agda:80:BoolHom : {ℓ ℓ' : Level} → (A : BooleanRing ℓ) → (B : BooleanRing ℓ') → Type _
Algebra\ChainComplex\Base.agda:19:record ChainComplex (ℓ : Level) : Type (ℓ-suc ℓ) where
Algebra\ChainComplex\Base.agda:26: (A : ChainComplex ℓ) (B : ChainComplex ℓ') : Type (ℓ-max ℓ ℓ') where
Algebra\ChainComplex\Base.agda:34:  (f g : ChainComplexMap A B) : Type (ℓ-max ℓ' ℓ) where
Algebra\ChainComplex\Base.agda:46:record CoChainComplex (ℓ : Level) : Type (ℓ-suc ℓ) where
Algebra\ChainComplex\Base.agda:84:  → ChainComplexMap A B  → Type (ℓ-max ℓ ℓ')
Algebra\ChainComplex\Base.agda:87:_≃Chain_ : (A : ChainComplex ℓ) (B : ChainComplex ℓ') → Type (ℓ-max ℓ ℓ')
Algebra\ChainComplex\Finite.agda:32:   (A : ChainComplex ℓ) (B : ChainComplex ℓ') : Type (ℓ-max ℓ ℓ') where
Algebra\ChainComplex\Finite.agda:43:    (f g : finChainComplexMap m A B) : Type (ℓ-max ℓ' ℓ) where
Algebra\ChainComplex\Finite.agda:90:    → finChainComplexMap m A B  → Type (ℓ-max ℓ ℓ')
Algebra\ChainComplex\Finite.agda:94:    → Type (ℓ-max ℓ ℓ')
Algebra\CommAlgebra\Base.agda:25:CommAlgebra : (R : CommRing ℓ) (ℓ' : Level) → Type _
Algebra\CommAlgebra\Base.agda:32:  ⟨_⟩ₐ : (A : CommAlgebra R ℓ') → Type ℓ'
Algebra\CommAlgebra\Base.agda:40:  record CommAlgebraStr (A : Type ℓ') : Type (ℓ-suc (ℓ-max ℓ ℓ')) where
Algebra\CommAlgebra\Base.agda:78:   This is defined as a record instead of a Σ-type to make type inference work
Algebra\CommAlgebra\Base.agda:81:  record IsCommAlgebraHom (A : CommAlgebra R ℓ') (B : CommAlgebra R ℓ'') (f : ⟨ A ⟩ₐ → ⟨ B ⟩ₐ) : Type
(ℓ-max ℓ (ℓ-max ℓ' ℓ'')) where
Algebra\CommAlgebra\Base.agda:110:  CommAlgebraHom : (A : CommAlgebra R ℓ') (B : CommAlgebra R ℓ'') → Type _
Algebra\CommAlgebra\Base.agda:174:  CommAlgebraEquiv : (A : CommAlgebra R ℓ') (B : CommAlgebra R ℓ'') → Type _
Algebra\CommAlgebra\Ideal.agda:20:  IdealsIn : Type _
Algebra\CommAlgebra\Localisation.agda:68:                            → Type (ℓ-suc ℓ)
Algebra\CommAlgebra\Polynomials.agda:13:open import Cubical.Algebra.CommRing.Polynomials.Typevariate as Poly hiding (var)
Algebra\CommAlgebra\Polynomials.agda:14:import Cubical.Algebra.CommRing.Polynomials.Typevariate.UniversalProperty
Algebra\CommAlgebra\Polynomials.agda:21:_[_]ₐ : (R : CommRing ℓ) (I : Type ℓ') → CommAlgebra R (ℓ-max ℓ ℓ')
Algebra\CommAlgebra\Polynomials.agda:29:module _ {R : CommRing ℓ} {I : Type ℓ'} where
Algebra\CommAlgebra\Polynomials.agda:52:evaluateAt : {R : CommRing ℓ} {I : Type ℓ'} (A : CommAlgebra R ℓ'')
Algebra\CommAlgebra\Polynomials.agda:58:homMapIso : {R : CommRing ℓ} {I : Type ℓ''} (A : CommAlgebra R ℓ')
Algebra\CommAlgebra\Polynomials.agda:68:  {R : CommRing ℓ} {I : Type ℓ'} (A : CommAlgebra R ℓ'') (φ : I → fst A )
Algebra\CommAlgebra\Polynomials.agda:74:homMapPath : {R : CommRing ℓ} {I : Type ℓ'} (A : CommAlgebra R (ℓ-max ℓ ℓ'))
Algebra\CommAlgebra\Polynomials.agda:79:equalByUMP : {R : CommRing ℓ} {I : Type ℓ'}
Algebra\CommAlgebra\Polynomials.agda:88:isIdByUMP : {R : CommRing ℓ} {I : Type ℓ'}
Algebra\CommAlgebra\Polynomials.agda:95:inducedHomVar : (R : CommRing ℓ) (I : Type ℓ')
Algebra\CommAlgebra\Polynomials.agda:112:  naturalEvR : {I : Type ℓ'} (ψ : CommAlgebraHom A B)
Algebra\CommAlgebra\Polynomials.agda:122:  natIndHomR : {I : Type ℓ'} (ψ : CommAlgebraHom A B)
Algebra\CommAlgebra\Polynomials.agda:136:  naturalEvL : {I J : Type ℓ'} (φ : J → I)
Algebra\CommAlgebra\Univalence.agda:44:                ≡⟨ fromPathP (funTypeTransp (λ _ → ⟨ R ⟩) ⟨_⟩ (uaCommRing e) (A .snd .fst)) ⟩
Algebra\CommAlgebra\AsModule\Base.agda:31:record IsCommAlgebra (R : CommRing ℓ) {A : Type ℓ'}
Algebra\CommAlgebra\AsModule\Base.agda:34:                     (_⋆_ : ⟨ R ⟩ → A → A) : Type (ℓ-max ℓ ℓ') where
Algebra\CommAlgebra\AsModule\Base.agda:45:record CommAlgebraStr (R : CommRing ℓ) (A : Type ℓ') : Type (ℓ-max ℓ ℓ') where
Algebra\CommAlgebra\AsModule\Base.agda:66:CommAlgebra : (R : CommRing ℓ) → ∀ ℓ' → Type (ℓ-max ℓ (ℓ-suc ℓ'))
Algebra\CommAlgebra\AsModule\Base.agda:67:CommAlgebra R ℓ' = Σ[ A ∈ Type ℓ' ] CommAlgebraStr R A
Algebra\CommAlgebra\AsModule\Base.agda:72:  CommAlgebraStr→AlgebraStr : {A : Type ℓ'} → CommAlgebraStr R A → AlgebraStr (CommRing→Ring
R) A
Algebra\CommAlgebra\AsModule\Base.agda:103:      {A : Type ℓ'} {0a 1a : A}
Algebra\CommAlgebra\AsModule\Base.agda:150:    (A : Type ℓ') (0a 1a : A)
Algebra\CommAlgebra\AsModule\Base.agda:204:  IsCommAlgebraEquiv : {A : Type ℓ'} {B : Type ℓ''}
Algebra\CommAlgebra\AsModule\Base.agda:206:    → Type _
Algebra\CommAlgebra\AsModule\Base.agda:210:  CommAlgebraEquiv : (M : CommAlgebra R ℓ') (N : CommAlgebra R ℓ'') → Type _
Algebra\CommAlgebra\AsModule\Base.agda:213:  IsCommAlgebraHom : {A : Type ℓ'} {B : Type ℓ''}
Algebra\CommAlgebra\AsModule\Base.agda:215:    → Type _
Algebra\CommAlgebra\AsModule\Base.agda:219:  CommAlgebraHom : (M : CommAlgebra R ℓ') (N : CommAlgebra R ℓ'') → Type _
Algebra\CommAlgebra\AsModule\Base.agda:282:isPropIsCommAlgebra : (R : CommRing ℓ) {A : Type ℓ'}
Algebra\CommAlgebra\AsModule\Ideal.agda:19:  IdealsIn : Type _
Algebra\CommAlgebra\AsModule\Localisation.agda:68:                        → Type (ℓ-suc ℓ)
Algebra\CommAlgebra\AsModule\LocalisationAlgebra.agda:140:                        → Type (ℓ-max ℓR (ℓ-suc ℓAlg))
Algebra\CommAlgebra\AsModule\LocalisationAlgebra.agda:180:        -- This is the template type for rings and ring morphisms satisfying
the
Algebra\CommAlgebra\AsModule\LocalisationAlgebra.agda:182:        type-univ : (S : CommRing ℓAlg) → CommRingHom Aᵣ S → Type _
Algebra\CommAlgebra\AsModule\LocalisationAlgebra.agda:183:        type-univ S f = ∃![ χ ∈ CommRingHom S (CommAlgebra→CommRing B) ]
Algebra\CommAlgebra\AsModule\LocalisationAlgebra.agda:186:        original-univ : type-univ RLoc.S⁻¹RAsCommRing RUniv./1AsCommRingHom
Algebra\CommAlgebra\AsModule\LocalisationAlgebra.agda:191:        univ : type-univ (CommAlgebra→CommRing S⁻¹AAsCommAlgebra)
Algebra\CommAlgebra\AsModule\LocalisationAlgebra.agda:194:          (sym $ cong₂ type-univ
Algebra\CommAlgebra\AsModule\LocalisationAlgebra.agda:202:          (λ i → type-univ (S⁻¹AAsCommAlgebra→CommRing (~ i))
Algebra\CommAlgebra\AsModule\LocalisationAlgebra.agda:206:          (λ i → type-univ (S⁻¹AAsCommAlgebra→CommRing (~ i))
Algebra\CommAlgebra\AsModule\LocalisationAlgebra.agda:213:        -- This is the template type for the _⋆_ preservation property we want
Algebra\CommAlgebra\AsModule\LocalisationAlgebra.agda:215:        type-pres⋆ : (S : CommRing ℓAlg) (_⋆_ : ⟨ R ⟩ → ⟨ S ⟩ → ⟨ S ⟩)
Algebra\CommAlgebra\AsModule\LocalisationAlgebra.agda:216:                   → (f : CommRingHom S (CommAlgebra→CommRing B)) → Type _
Algebra\CommAlgebra\AsModule\LocalisationAlgebra.agda:217:        type-pres⋆ S _S⋆_ f =
Algebra\CommAlgebra\AsModule\LocalisationAlgebra.agda:221:        original-pres⋆ : type-pres⋆ RLoc.S⁻¹RAsCommRing _⋆_ (original-univ
.fst .fst)
Algebra\CommAlgebra\AsModule\LocalisationAlgebra.agda:228:        pres⋆ : type-pres⋆ (CommAlgebra→CommRing S⁻¹AAsCommAlgebra) _⋆_ (univ
.fst .fst)
Algebra\CommAlgebra\AsModule\LocalisationAlgebra.agda:230:          (λ i → type-pres⋆ (S⁻¹AAsCommAlgebra→CommRing (~ i))
Algebra\CommAlgebra\AsModule\Properties.agda:66: CommRingWithHom : Type (ℓ-max ℓ (ℓ-suc ℓ'))
Algebra\CommAlgebra\AsModule\Properties.agda:147: isCommRingWithHomHom : (A B : CommRingWithHom) → CommRingHom (fst A) (fst B) → Type
(ℓ-max ℓ ℓ')
Algebra\CommAlgebra\AsModule\Properties.agda:150: CommRingWithHomHom : CommRingWithHom → CommRingWithHom → Type (ℓ-max ℓ ℓ')
Algebra\CommAlgebra\AsModule\Properties.agda:182: isCommRingWithHomEquiv : (A B : CommRingWithHom) → CommRingEquiv (fst A) (fst B) →
Type (ℓ-max ℓ ℓ')
Algebra\CommAlgebra\AsModule\Properties.agda:185: CommRingWithHomEquiv : CommRingWithHom → CommRingWithHom → Type (ℓ-max ℓ ℓ')
Algebra\CommAlgebra\AsModule\Properties.agda:236:  isSetCommAlgStr : (A : Type ℓ') → isSet (CommAlgebraStr R A)
Algebra\CommAlgebra\AsModule\Properties.agda:271:recPT→CommAlgebra : {R : CommRing ℓ} {A : Type ℓ'} (𝓕  : A → CommAlgebra R ℓ'')
Algebra\CommAlgebra\AsModule\Properties.agda:283:contrCommAlgebraHom→contrCommAlgebraEquiv : {R : CommRing ℓ} {A : Type ℓ'}
Algebra\CommAlgebra\AsModule\QuotientAlgebra.agda:34:  long type checking times. All other definitions that need to "look into" this
Algebra\CommAlgebra\AsModule\FreeCommAlgebra\Base.agda:15:  * the definition of the free commutative algebra on a type I over a
commutative ring R as a HIT
Algebra\CommAlgebra\AsModule\FreeCommAlgebra\Base.agda:32:  data R[_] (I : Type ℓ') : Type (ℓ-max ℓ ℓ') where
Algebra\CommAlgebra\AsModule\FreeCommAlgebra\Base.agda:55:  _⋆_ : {I : Type ℓ'} → fst R → R[ I ] → R[ I ]
Algebra\CommAlgebra\AsModule\FreeCommAlgebra\Base.agda:58:  ⋆-assoc : {I : Type ℓ'} → (s t : fst R) (x : R[ I ]) → (s ·r t) ⋆ x ≡ s ⋆
(t ⋆ x)
Algebra\CommAlgebra\AsModule\FreeCommAlgebra\Base.agda:64:  ⋆-ldist-+ : {I : Type ℓ'} → (s t : fst R) (x : R[ I ]) → (s +r t) ⋆ x ≡ (s
⋆ x) + (t ⋆ x)
Algebra\CommAlgebra\AsModule\FreeCommAlgebra\Base.agda:69:  ⋆-rdist-+ : {I : Type ℓ'} → (s : fst R) (x y : R[ I ]) → s ⋆ (x + y) ≡ (s ⋆
x) + (s ⋆ y)
Algebra\CommAlgebra\AsModule\FreeCommAlgebra\Base.agda:76:  ⋆-assoc-· : {I : Type ℓ'} → (s : fst R) (x y : R[ I ]) → (s ⋆ x) · y ≡ s ⋆
(x · y)
Algebra\CommAlgebra\AsModule\FreeCommAlgebra\Base.agda:80:  0a : {I : Type ℓ'} → R[ I ]
Algebra\CommAlgebra\AsModule\FreeCommAlgebra\Base.agda:83:  1a : {I : Type ℓ'} → R[ I ]
Algebra\CommAlgebra\AsModule\FreeCommAlgebra\Base.agda:86:  isCommAlgebra : {I : Type ℓ'} → IsCommAlgebra R {A = R[ I ]} 0a 1a _+_ _·_
-_ _⋆_
Algebra\CommAlgebra\AsModule\FreeCommAlgebra\Base.agda:92:_[_] : (R : CommRing ℓ) (I : Type ℓ') → CommAlgebra R (ℓ-max ℓ ℓ')
Algebra\CommAlgebra\AsModule\FreeCommAlgebra\OnCoproduct.agda:2: The goal of this module is to show that for two types I,J, there is an
Algebra\CommAlgebra\AsModule\FreeCommAlgebra\OnCoproduct.agda:31:module CalculateFreeCommAlgebraOnCoproduct (R : CommRing ℓ) (I J :
Type ℓ) where
Algebra\CommAlgebra\AsModule\FreeCommAlgebra\OnCoproduct.agda:150:                         (substRefl {B = λ (X : Type ℓ) → ⟨ R[I][J] ⟩
→ X} (fst from'))
Algebra\CommAlgebra\AsModule\FreeCommAlgebra\OnCoproduct.agda:159:                         (substRefl {B = λ (X : Type ℓ) → ⟨ R[I][J] ⟩
→ X} (fst from'))
Algebra\CommAlgebra\AsModule\FreeCommAlgebra\OnCoproduct.agda:234:module _ {R : CommRing ℓ} {I J : Type ℓ} where
Algebra\CommAlgebra\AsModule\FreeCommAlgebra\Properties.agda:45:module Theory {R : CommRing ℓ} {I : Type ℓ'} where
Algebra\CommAlgebra\AsModule\FreeCommAlgebra\Properties.agda:57:    {P : ⟨ R [ I ] ⟩ → Type ℓ''}
Algebra\CommAlgebra\AsModule\FreeCommAlgebra\Properties.agda:266:evaluateAt : {R : CommRing ℓ} {I : Type ℓ'} (A : CommAlgebra R ℓ'')
Algebra\CommAlgebra\AsModule\FreeCommAlgebra\Properties.agda:271:inducedHom : {R : CommRing ℓ} {I : Type ℓ'} (A : CommAlgebra R ℓ'')
Algebra\CommAlgebra\AsModule\FreeCommAlgebra\Properties.agda:277:homMapIso : {R : CommRing ℓ} {I : Type ℓ''} (A : CommAlgebra R ℓ')
Algebra\CommAlgebra\AsModule\FreeCommAlgebra\Properties.agda:287:  {R : CommRing ℓ} {I : Type ℓ'} (A : CommAlgebra R ℓ'') (φ : I → fst
A )
Algebra\CommAlgebra\AsModule\FreeCommAlgebra\Properties.agda:293:homMapPath : {R : CommRing ℓ} {I : Type ℓ'} (A : CommAlgebra R (ℓ-max
ℓ ℓ'))
Algebra\CommAlgebra\AsModule\FreeCommAlgebra\Properties.agda:298:equalByUMP : {R : CommRing ℓ} {I : Type ℓ'}
Algebra\CommAlgebra\AsModule\FreeCommAlgebra\Properties.agda:307:isIdByUMP : {R : CommRing ℓ} {I : Type ℓ'}
Algebra\CommAlgebra\AsModule\FreeCommAlgebra\Properties.agda:314:inducedHomVar : (R : CommRing ℓ) (I : Type ℓ')
Algebra\CommAlgebra\AsModule\FreeCommAlgebra\Properties.agda:331:  naturalEvR : {I : Type ℓ'} (ψ : CommAlgebraHom A B)
Algebra\CommAlgebra\AsModule\FreeCommAlgebra\Properties.agda:341:  natIndHomR : {I : Type ℓ'} (ψ : CommAlgebraHom A B)
Algebra\CommAlgebra\AsModule\FreeCommAlgebra\Properties.agda:355:  naturalEvL : {I J : Type ℓ'} (φ : J → I)
Algebra\CommAlgebra\AsModule\FreeCommAlgebra\Properties.agda:389:module _ {R : CommRing ℓ} {I : Type ℓ} where
Algebra\CommAlgebra\AsModule\Instances\Pointwise.agda:15:module _ {R : CommRing ℓ} (X : Type ℓ'') (A : CommAlgebra R ℓ') where
Algebra\CommAlgebra\FP\Base.agda:33:  record FinitePresentation : Type ℓ where
Algebra\CommAlgebra\FP\Base.agda:67:  FPsOf : (A : CommAlgebra R ℓ') → Type (ℓ-max ℓ ℓ')
Algebra\CommAlgebra\FP\Base.agda:70:  isFP : (A : CommAlgebra R ℓ') → Type (ℓ-max ℓ ℓ')
Algebra\CommAlgebra\FP\Base.agda:77:  FPCAlg : (ℓ' : Level) → Type _
Algebra\CommMonoid\Base.agda:28:record IsCommMonoid {M : Type ℓ}
Algebra\CommMonoid\Base.agda:29:                    (ε : M) (_·_ : M → M → M) : Type ℓ where
Algebra\CommMonoid\Base.agda:40:record CommMonoidStr (M : Type ℓ) : Type ℓ where
Algebra\CommMonoid\Base.agda:52:CommMonoid : ∀ ℓ → Type (ℓ-suc ℓ)
Algebra\CommMonoid\Base.agda:53:CommMonoid ℓ = TypeWithStr ℓ CommMonoidStr
Algebra\CommMonoid\Base.agda:55:makeIsCommMonoid : {M : Type ℓ} {ε : M} {_·_ : M → M → M}
Algebra\CommMonoid\Base.agda:65:makeCommMonoid : {M : Type ℓ} (ε : M) (_·_ : M → M → M)
Algebra\CommMonoid\Base.agda:77:CommMonoidStr→MonoidStr : {A : Type ℓ} → CommMonoidStr A → MonoidStr A
Algebra\CommMonoid\Base.agda:83:CommMonoidHom : (L : CommMonoid ℓ) (M : CommMonoid ℓ') → Type (ℓ-max ℓ ℓ')
Algebra\CommMonoid\Base.agda:86:IsCommMonoidEquiv : {A : Type ℓ} {B : Type ℓ'}
Algebra\CommMonoid\Base.agda:87:  (M : CommMonoidStr A) (e : A ≃ B) (N : CommMonoidStr B) → Type (ℓ-max ℓ ℓ')
Algebra\CommMonoid\Base.agda:90:CommMonoidEquiv : (M : CommMonoid ℓ) (N : CommMonoid ℓ') → Type (ℓ-max ℓ ℓ')
Algebra\CommMonoid\Base.agda:93:isPropIsCommMonoid : {M : Type ℓ} (ε : M) (_·_ : M → M → M)
Algebra\CommMonoid\GrothendieckGroup.agda:41:  R : ⟨ M² ⟩  → ⟨ M² ⟩ → Type _
Algebra\CommMonoid\GrothendieckGroup.agda:44:  M²/R : Type _
Algebra\CommMonoid\Properties.agda:26:      subtype = Σ[ x ∈ ⟨ M ⟩ ] ⟨ P x ⟩
Algebra\CommMonoid\Properties.agda:29:    fst makeSubCommMonoid = subtype
Algebra\CommMonoid\Properties.agda:60: hasInverse : (x : M) → Type ℓ
Algebra\CommMonoid\Instances\FreeComMonoid.agda:11:FCMCommMonoid : {A : Type ℓ} → CommMonoid ℓ
Algebra\CommRing\Base.agda:28:record IsCommRing {R : Type ℓ}
Algebra\CommRing\Base.agda:29:                  (0r 1r : R) (_+_ _·_ : R → R → R) (-_ : R → R) : Type ℓ where
Algebra\CommRing\Base.agda:41:record CommRingStr (A : Type ℓ) : Type (ℓ-suc ℓ) where
Algebra\CommRing\Base.agda:59:CommRing : ∀ ℓ → Type (ℓ-suc ℓ)
Algebra\CommRing\Base.agda:60:CommRing ℓ = TypeWithStr ℓ CommRingStr
Algebra\CommRing\Base.agda:63:makeIsCommRing : {R : Type ℓ} {0r 1r : R} {_+_ _·_ : R → R → R} { -_ : R → R}
Algebra\CommRing\Base.agda:79:makeCommRing : {R : Type ℓ} (0r 1r : R) (_+_ _·_ : R → R → R) (-_ : R → R)
Algebra\CommRing\Base.agda:93:CommRingStr→RingStr : {A : Type ℓ} → CommRingStr A → RingStr A
Algebra\CommRing\Base.agda:123:record IsCommRingHom {A : Type ℓ} {B : Type ℓ'} (R : CommRingStr A) (f : A → B) (S : CommRingStr B)
Algebra\CommRing\Base.agda:124:  : Type (ℓ-max ℓ ℓ')
Algebra\CommRing\Base.agda:140:CommRingHom : (R : CommRing ℓ) (S : CommRing ℓ') → Type (ℓ-max ℓ ℓ')
Algebra\CommRing\Base.agda:143:IsCommRingEquiv : {A : Type ℓ} {B : Type ℓ'}
Algebra\CommRing\Base.agda:144:  (R : CommRingStr A) (e : A ≃ B) (S : CommRingStr B) → Type (ℓ-max ℓ ℓ')
Algebra\CommRing\Base.agda:147:CommRingEquiv : (R : CommRing ℓ) (S : CommRing ℓ') → Type (ℓ-max ℓ ℓ')
Algebra\CommRing\Base.agda:153:isPropIsCommRing : {R : Type ℓ} (0r 1r : R) (_+_ _·_ : R → R → R) (-_ : R → R)
Algebra\CommRing\Base.agda:162:isPropIsCommRingHom : {A : Type ℓ} {B : Type ℓ'} (R : CommRingStr A) (f : A → B) (S : CommRingStr B)
Algebra\CommRing\FGIdeal.agda:47:  _holds : hProp ℓ → Type ℓ
Algebra\CommRing\FGIdeal.agda:68:  isLinearCombination : {n : ℕ} → FinVec R n → R → Type ℓ
Algebra\CommRing\FGIdeal.agda:218:FGIdealIn : (R : CommRing ℓ) → Type (ℓ-suc ℓ)
Algebra\CommRing\FiberedProduct.agda:28:  fbT : Type ℓ
Algebra\CommRing\LocalRing.agda:50:  isLocal : Type ℓ
Algebra\CommRing\LocalRing.agda:137:      BinSum : Type ℓ
Algebra\CommRing\LocalRing.agda:140:      Alternative : Type ℓ
Algebra\CommRing\LocalRing.agda:189:      OneMinus : Type ℓ
Algebra\CommRing\LocalRing.agda:192:      Alternative : Type ℓ
Algebra\CommRing\Properties.agda:377:recPT→CommRing : {A : Type ℓ'} (𝓕  : A → CommRing ℓ)
Algebra\CommRing\Univalence.agda:52:CommRingIso : (R : CommRing ℓ) (S : CommRing ℓ') → Type (ℓ-max ℓ ℓ')
Algebra\CommRing\Univalence.agda:82:module _ (R : CommRing ℓ) {A : Type ℓ}
Algebra\CommRing\Univalence.agda:97:    BaseΣ : Type (ℓ-suc ℓ)
Algebra\CommRing\Univalence.agda:98:    BaseΣ = Σ[ B ∈ Type ℓ ] B × B × (B → B → B) × (B → B → B) × (B → B)
Algebra\CommRing\Univalence.agda:100:    FamilyΣ : BaseΣ → Type ℓ
Algebra\CommRing\Ideal\Base.agda:43:  record isCommIdeal (I : ℙ R) : Type ℓ where
Algebra\CommRing\Ideal\Base.agda:62:  CommIdeal : Type (ℓ-suc ℓ)
Algebra\CommRing\Ideal\Base.agda:69:  _⊆_ : CommIdeal → CommIdeal → Type ℓ
Algebra\CommRing\Ideal\Base.agda:73:  _∈_ : R → CommIdeal → Type ℓ
Algebra\CommRing\Ideal\Base.agda:113:IdealsIn : (R : CommRing ℓ) → Type _
Algebra\CommRing\Instances\Pointwise.agda:12:pointwiseRing : (X : Type ℓ) (R : CommRing ℓ') → CommRing _
Algebra\CommRing\Instances\QuoInt.agda:14:  renaming (ℤ to ℤType ; _+_ to _+ℤ_; _·_ to _·ℤ_; -_ to -ℤ_)
Algebra\CommRing\Instances\QuoInt.agda:25:+ℤInvL : (n : ℤType) → (-ℤ n) +ℤ n ≡ 0
Algebra\CommRing\Instances\QuoInt.agda:36:+ℤInvR : (n : ℤType) → n +ℤ (-ℤ n) ≡ 0
Algebra\CommRing\Instances\QuoInt.agda:41:ℤCommRing .fst = ℤType
Algebra\CommRing\Instances\QuoQRationals.agda:12:  renaming (ℚ to ℚType ; _+_ to _+ℚ_; _·_ to _·ℚ_; -_ to -ℚ_)
Algebra\CommRing\Instances\QuoQRationals.agda:18:ℚCommRing .fst = ℚType
Algebra\CommRing\Localisation\Base.agda:42:    A : Type ℓ
Algebra\CommRing\Localisation\Base.agda:46:record isMultClosedSubset (R' : CommRing ℓ) (S' : ℙ (fst R')) : Type ℓ where
Algebra\CommRing\Localisation\Base.agda:64: _≈_ : R × S → R × S → Type ℓ
Algebra\CommRing\Localisation\InvertingElements.agda:60:    A : Type ℓ
Algebra\CommRing\Localisation\InvertingElements.agda:81: R[1/_] : R → Type ℓ
Algebra\CommRing\Localisation\InvertingElements.agda:118: invElPropElim : {f : R} {P : R[1/ f ] → Type ℓ'}
Algebra\CommRing\Localisation\InvertingElements.agda:133: invElPropElim2 : {f g : R} {P : R[1/ f ] → R[1/ g ] → Type ℓ'}
Algebra\CommRing\Localisation\InvertingElements.agda:179: invElPropElimN : (n : ℕ) (f : FinVec R n) (P : ((i : Fin n) → R[1/ (f i) ]) →
Type ℓ')
Algebra\CommRing\Localisation\InvertingElements.agda:191:   P' : ((i : Fin n) → R[1/ (f (suc i)) ]) → Type _
Algebra\CommRing\Localisation\InvertingElements.agda:200:     P'' : R[1/ (f zero) ] → Type _
Algebra\CommRing\Localisation\InvertingElements.agda:251: powersPropElim : {f : R} {P : R → Type ℓ'}
Algebra\CommRing\Localisation\Limit.agda:205:  χ≡Elim<Only x <hyp i j = aux (i ≟ j) -- doesn't type check in reasonable time using with
syntax
Algebra\CommRing\Localisation\Limit.agda:236: χ≡PropElim : {B : ((i : Fin n) → R[1/ f i ]) → Type ℓ''} (isPropB : ∀ {x} → isProp (B x))
Algebra\CommRing\Localisation\PullbackSquare.agda:71:    A : Type ℓ
Algebra\CommRing\Localisation\UniversalProperty.agda:60:                     → Type (ℓ-suc ℓ)
Algebra\CommRing\Localisation\UniversalProperty.agda:357: record PathToS⁻¹R (A' : CommRing ℓ) (φ : CommRingHom R' A') : Type ℓ where
Algebra\CommRing\Polynomials\MultivariatePoly-notationZ.agda:27:ℤ[x] : Type ℓ-zero
Algebra\CommRing\Polynomials\MultivariatePoly-notationZ.agda:33:ℤ[x,y] : Type ℓ-zero
Algebra\CommRing\Polynomials\MultivariatePoly-notationZ.agda:39:ℤ[x,y,z] : Type ℓ-zero
Algebra\CommRing\Polynomials\MultivariatePoly-notationZ.agda:45:ℤ[x1,···,xn] : (n : ℕ) → Type ℓ-zero
Algebra\CommRing\Polynomials\MultivariatePoly-notationZ.agda:67:ℤ[x]/x : Type ℓ-zero
Algebra\CommRing\Polynomials\MultivariatePoly-notationZ.agda:73:ℤ[x]/x² : Type ℓ-zero
Algebra\CommRing\Polynomials\MultivariatePoly-notationZ.agda:79:ℤ[x]/x³ : Type ℓ-zero
Algebra\CommRing\Polynomials\MultivariatePoly-notationZ.agda:85:ℤ[x1,···,xn]/<x1,···,xn> : (n : ℕ) → Type ℓ-zero
Algebra\CommRing\Polynomials\MultivariatePoly-notationZ2.agda:27:ℤ/2[x] : Type ℓ-zero
Algebra\CommRing\Polynomials\MultivariatePoly-notationZ2.agda:33:ℤ/2[x,y] : Type ℓ-zero
Algebra\CommRing\Polynomials\MultivariatePoly-notationZ2.agda:39:ℤ/2[x,y,z] : Type ℓ-zero
Algebra\CommRing\Polynomials\MultivariatePoly-notationZ2.agda:45:ℤ/2[x1,···,xn] : (n : ℕ) → Type ℓ-zero
Algebra\CommRing\Polynomials\MultivariatePoly-Quotient.agda:46:  A[x1,···,xn]/<xkʲ> : (k j : ℕ) → Type ℓ
Algebra\CommRing\Polynomials\MultivariatePoly-Quotient.agda:55:  A[x1,···,xn]/<x1,···,xn> : Type ℓ
Algebra\CommRing\Polynomials\MultivariatePoly.agda:58:  Poly : Type ℓ
Algebra\CommRing\Polynomials\Typevariate.agda:2:module Cubical.Algebra.CommRing.Polynomials.Typevariate where
Algebra\CommRing\Polynomials\Typevariate.agda:4:open import Cubical.Algebra.CommRing.Polynomials.Typevariate.Base public
Algebra\CommRing\Polynomials\Typevariate.agda:5:open import Cubical.Algebra.CommRing.Polynomials.Typevariate.Properties public
Algebra\CommRing\Polynomials\Typevariate\Base.agda:2:module Cubical.Algebra.CommRing.Polynomials.Typevariate.Base where
Algebra\CommRing\Polynomials\Typevariate\Base.agda:15:  * the definition of the free commutative algebra on a type I over a commutative
ring R as a HIT
Algebra\CommRing\Polynomials\Typevariate\Base.agda:33:  data R[_] (I : Type ℓ') : Type (ℓ-max ℓ ℓ') where
Algebra\CommRing\Polynomials\Typevariate\Base.agda:57:    isCommRing : (I : Type ℓ') → IsCommRing (const {I = I} 0r) (const 1r) _+_ _·_
-_
Algebra\CommRing\Polynomials\Typevariate\Base.agda:68:  module _ (I : Type ℓ') where
Algebra\CommRing\Polynomials\Typevariate\Base.agda:81:    constIsCommRingHom : (I : Type ℓ') → IsCommRingHom (R .snd) (const {I = I})
(commRingStr I)
Algebra\CommRing\Polynomials\Typevariate\Base.agda:84:_[_] : (R : CommRing ℓ) (I : Type ℓ') → CommRing (ℓ-max ℓ ℓ')
Algebra\CommRing\Polynomials\Typevariate\Base.agda:88:constPolynomial : (R : CommRing ℓ) (I : Type ℓ') → CommRingHom R (R [ I ])
Algebra\CommRing\Polynomials\Typevariate\Base.agda:94:  var : {R : CommRing ℓ} {I : Type ℓ'} → I → ⟨ R [ I ] ⟩
Algebra\CommRing\Polynomials\Typevariate\OnCoproduct.agda:2: The goal of this module is to show that for two types I,J, there is an
Algebra\CommRing\Polynomials\Typevariate\OnCoproduct.agda:9:module Cubical.Algebra.CommRing.Polynomials.Typevariate.OnCoproduct where
Algebra\CommRing\Polynomials\Typevariate\OnCoproduct.agda:19:open import Cubical.Algebra.CommRing.Polynomials.Typevariate.Base
Algebra\CommRing\Polynomials\Typevariate\OnCoproduct.agda:20:open import
Cubical.Algebra.CommRing.Polynomials.Typevariate.UniversalProperty
Algebra\CommRing\Polynomials\Typevariate\OnCoproduct.agda:26:module CalculatePolynomialsOnCoproduct (R : CommRing ℓ) (I J : Type ℓ)
where
Algebra\CommRing\Polynomials\Typevariate\Properties.agda:1:module Cubical.Algebra.CommRing.Polynomials.Typevariate.Properties where
Algebra\CommRing\Polynomials\Typevariate\Properties.agda:15:open import Cubical.Algebra.CommRing.Polynomials.Typevariate.Base
Algebra\CommRing\Polynomials\Typevariate\Properties.agda:16:open import
Cubical.Algebra.CommRing.Polynomials.Typevariate.UniversalProperty
Algebra\CommRing\Polynomials\Typevariate\UniversalProperty.agda:1:module
Cubical.Algebra.CommRing.Polynomials.Typevariate.UniversalProperty where
Algebra\CommRing\Polynomials\Typevariate\UniversalProperty.agda:21:open import Cubical.Algebra.CommRing.Polynomials.Typevariate.Base
Algebra\CommRing\Polynomials\Typevariate\UniversalProperty.agda:33:module _ {R : CommRing ℓ} {I : Type ℓ'} where
Algebra\CommRing\Polynomials\Typevariate\UniversalProperty.agda:46:    {P : ⟨ R [ I ] ⟩ → Type ℓ''}
Algebra\CommRing\Polynomials\Typevariate\UniversalProperty.agda:212:module _  {R : CommRing ℓ} {I : Type ℓ'} (S : CommRing ℓ'') (f :
CommRingHom R S) where
Algebra\CommRing\Polynomials\Typevariate\UniversalProperty.agda:279:  idByIdOnVars : {R : CommRing ℓ} {I : Type ℓ'}
Algebra\CommRing\Quotient\IdealSum.agda:120:  {- workaround for slow type checking, more specifically,
Algebra\CommRing\Quotient\ImageQuotient.agda:19:module _ {ℓ : Level} (R : CommRing ℓ) {X : Type ℓ} (f : X → ⟨ R ⟩) where
Algebra\CommRing\Quotient\ImageQuotient.agda:26:    data generatedIdeal : ⟨ R ⟩ → Type ℓ where
Algebra\CommRing\Quotient\ImageQuotient.agda:140:         {X : Type ℓ} {f : X → ⟨ R ⟩}  where
Algebra\CommRing\Quotient\ImageQuotient.agda:142:    quotientImageMapEpi : {ℓ' : Level} → {S : Type ℓ'} →
Algebra\CommRing\Quotient\ImageQuotient.agda:158:  evalInduce : {ℓ : Level} (R : CommRing ℓ) {X : Type ℓ} {f : X → ⟨ R ⟩}
Algebra\CommSemiring\Base.agda:4:open import Cubical.Foundations.SIP using (TypeWithStr)
Algebra\CommSemiring\Base.agda:14:record IsCommSemiring {R : Type ℓ}
Algebra\CommSemiring\Base.agda:15:                  (0r 1r : R) (_+_ _·_ : R → R → R) : Type ℓ where
Algebra\CommSemiring\Base.agda:23:record CommSemiringStr (A : Type ℓ) : Type (ℓ-suc ℓ) where
Algebra\CommSemiring\Base.agda:37:CommSemiring : ∀ ℓ → Type (ℓ-suc ℓ)
Algebra\CommSemiring\Base.agda:38:CommSemiring ℓ = TypeWithStr ℓ CommSemiringStr
Algebra\CommSemiring\Base.agda:58:  {R : Type ℓ} {0r 1r : R} {_+_ _·_ : R → R → R}
Algebra\CommSemiring\Instances\UpperNat.agda:53:   This is different for the subtype of *bounded* upper naturals ℕ↑b.
Algebra\CommSemiring\Instances\UpperNat.agda:85:  ℕ↑ : Type₁
Algebra\CommSemiring\Instances\UpperNat.agda:89:    using (typeAt; pathFromImplications)
Algebra\CommSemiring\Instances\UpperNat.agda:100:      ⇒ : (n : ℕ) → typeAt n (x · (y + z))  → typeAt n ((x · y) + (x · z))
Algebra\CommSemiring\Instances\UpperNat.agda:167:    using (typeAt; pathFromImplications)
Algebra\CommSemiring\Instances\UpperNat.agda:169:  ℕ↑b : Type₁
Algebra\CommSemiring\Instances\UpperNat.agda:177:     ⇒ : (n : ℕ) → typeAt n (fst (0↑ · x)) → typeAt n (fst 0↑)
Algebra\CommSemiring\Instances\UpperNat.agda:179:     ⇐ : (n : ℕ) → typeAt n (fst 0↑) → typeAt n (fst (0↑ · x))
Algebra\DirectSum\Equiv-DSHIT-DSFun.agda:46:  (G : ℕ → Type ℓ)
Algebra\DirectSum\Equiv-DSHIT-DSFun.agda:406:  (G : ℕ → Type ℓ)
Algebra\DirectSum\DirectSumFun\Base.agda:22:AlmostNullProof : (G : ℕ → Type ℓ) → (Gstr : (n : ℕ) → AbGroupStr (G n))
Algebra\DirectSum\DirectSumFun\Base.agda:23:                  → (f : (n : ℕ) → G n) → (k : ℕ) → Type ℓ
Algebra\DirectSum\DirectSumFun\Base.agda:26:AlmostNull : (G : ℕ → Type ℓ) → (Gstr : (n : ℕ) → AbGroupStr (G n))
Algebra\DirectSum\DirectSumFun\Base.agda:27:             → (f : (n : ℕ) → G n) → Type ℓ
Algebra\DirectSum\DirectSumFun\Base.agda:30:AlmostNullP : (G : ℕ → Type ℓ) → (Gstr : (n : ℕ) → AbGroupStr (G n))
Algebra\DirectSum\DirectSumFun\Base.agda:31:             → (f : (n : ℕ) → G n) → Type ℓ
Algebra\DirectSum\DirectSumFun\Base.agda:34:⊕Fun : (G : ℕ → Type ℓ) → (Gstr : (n : ℕ) → AbGroupStr (G n)) → Type ℓ
Algebra\DirectSum\DirectSumFun\Properties.agda:26:  (G : ℕ → Type ℓ)
Algebra\DirectSum\DirectSumHIT\Base.agda:20:data ⊕HIT (Idx : Type ℓ) (P : Idx → Type ℓ') (AGP : (r : Idx) → AbGroupStr (P r)) : Type
(ℓ-max ℓ ℓ')  where
Algebra\DirectSum\DirectSumHIT\Base.agda:37:module _ (Idx : Type ℓ) (P : Idx → Type ℓ') (AGP : (r : Idx) → AbGroupStr (P r)) where
Algebra\DirectSum\DirectSumHIT\Base.agda:40:    (Q            : (x : ⊕HIT Idx P AGP) → Type ℓ'')
Algebra\DirectSum\DirectSumHIT\Base.agda:73:    (B : Type ℓ'')
Algebra\DirectSum\DirectSumHIT\Base.agda:91:    (Q            : (x : ⊕HIT Idx P AGP) → Type ℓ'')
Algebra\DirectSum\DirectSumHIT\Base.agda:110:    (B        : Type ℓ'')
Algebra\DirectSum\DirectSumHIT\Properties.agda:18:  (Idx : Type ℓ)
Algebra\DirectSum\DirectSumHIT\Properties.agda:19:  (P : Idx → Type ℓ')
Algebra\DirectSum\DirectSumHIT\Properties.agda:66:  (Idx : Type ℓ)
Algebra\DirectSum\DirectSumHIT\Properties.agda:67:  (G    : Idx → Type ℓ')
Algebra\DirectSum\DirectSumHIT\Properties.agda:88:  (Idx  : Type ℓ)
Algebra\DirectSum\DirectSumHIT\Properties.agda:90:  (G    : Idx → Type ℓ')
Algebra\DirectSum\DirectSumHIT\PseudoNormalForm.agda:28:  (G : (n : ℕ) → Type ℓ)
Algebra\DirectSum\DirectSumHIT\PseudoNormalForm.agda:107:  untruncatedPNF : (x : ⊕HIT ℕ G Gstr) → Type ℓ
Algebra\DirectSum\DirectSumHIT\PseudoNormalForm.agda:110:  PNF : (x : ⊕HIT ℕ G Gstr) → Type ℓ
Algebra\DirectSum\DirectSumHIT\PseudoNormalForm.agda:113:  untruncatedPNF2 : (x y : ⊕HIT ℕ G Gstr) → Type ℓ
Algebra\DirectSum\DirectSumHIT\PseudoNormalForm.agda:116:  PNF2 :  (x y : ⊕HIT ℕ G Gstr) → Type ℓ
Algebra\DirectSum\DirectSumHIT\PseudoNormalForm.agda:169:   The issue with the current one is that we rely on a underline data type
depVec
Algebra\DirectSum\DirectSumHIT\UniversalProperty.agda:25:  (Idx              : Type ℓ)
Algebra\DirectSum\DirectSumHIT\UniversalProperty.agda:26:  (G                : (k : Idx) → Type ℓ')
Algebra\DistLattice\Base.agda:31:record IsDistLattice {L : Type ℓ}
Algebra\DistLattice\Base.agda:32:                     (0l 1l : L) (_∨l_ _∧l_ : L → L → L) : Type ℓ where
Algebra\DistLattice\Base.agda:57:record DistLatticeStr (A : Type ℓ) : Type (ℓ-suc ℓ) where
Algebra\DistLattice\Base.agda:73:DistLattice : ∀ ℓ → Type (ℓ-suc ℓ)
Algebra\DistLattice\Base.agda:74:DistLattice ℓ = TypeWithStr ℓ DistLatticeStr
Algebra\DistLattice\Base.agda:81:makeIsDistLattice∧lOver∨l : {L : Type ℓ} {0l 1l : L} {_∨l_ _∧l_ : L → L → L}
Algebra\DistLattice\Base.agda:128:makeDistLattice∧lOver∨l : {L : Type ℓ} (0l 1l : L) (_∨l_ _∧l_ : L → L → L)
Algebra\DistLattice\Base.agda:147:makeIsDistLattice∨lOver∧l : {L : Type ℓ} {0l 1l : L} {_∨l_ _∧l_ : L → L → L}
Algebra\DistLattice\Base.agda:195:makeDistLattice∨lOver∧l : {L : Type ℓ} (0l 1l : L) (_∨l_ _∧l_ : L → L → L)
Algebra\DistLattice\Base.agda:214:DistLatticeStr→LatticeStr : {A : Type ℓ} → DistLatticeStr A → LatticeStr A
Algebra\DistLattice\Base.agda:222:DistLatticeHom : (L : DistLattice ℓ) (M : DistLattice ℓ') → Type (ℓ-max ℓ ℓ')
Algebra\DistLattice\Base.agda:228:IsDistLatticeEquiv : {A : Type ℓ} {B : Type ℓ'}
Algebra\DistLattice\Base.agda:229:  (L : DistLatticeStr A) (e : A ≃ B) (M : DistLatticeStr B) → Type (ℓ-max ℓ ℓ')
Algebra\DistLattice\Base.agda:233:DistLatticeEquiv : (L : DistLattice ℓ) (M : DistLattice ℓ') → Type (ℓ-max ℓ ℓ')
Algebra\DistLattice\Base.agda:236:isPropIsDistLattice : {L : Type ℓ} (0l 1l : L) (_∨l_ _∧l_ : L → L → L)
Algebra\DistLattice\Basis.agda:45: record IsGenSublattice (M : Semilattice ℓ) (e : fst M → L) : Type ℓ where
Algebra\DistLattice\Basis.agda:57: record IsBasis (S : ℙ L) : Type ℓ where
Algebra\Field\Base.agda:7:   type has decidable equality.
Algebra\Field\Base.agda:43:record IsField {R : Type ℓ}
Algebra\Field\Base.agda:44:                  (0r 1r : R) (_+_ _·_ : R → R → R) (-_ : R → R) : Type ℓ where
Algebra\Field\Base.agda:62:record FieldStr (A : Type ℓ) : Type (ℓ-suc ℓ) where
Algebra\Field\Base.agda:81:Field : ∀ ℓ → Type (ℓ-suc ℓ)
Algebra\Field\Base.agda:82:Field ℓ = TypeWithStr ℓ FieldStr
Algebra\Field\Base.agda:85:makeIsField : {R : Type ℓ} {0r 1r : R} {_+_ _·_ : R → R → R} { -_ : R → R}
Algebra\Field\Base.agda:106:makeField : {R : Type ℓ} (0r 1r : R) (_+_ _·_ : R → R → R) (-_ : R → R) (_[_]⁻¹ : (x : R) → ¬ (x ≡ 0r) → R)
Algebra\Field\Base.agda:135:FieldStr→CommRingStr : {A : Type ℓ} → FieldStr A → CommRingStr A
Algebra\Field\Base.agda:142:record IsFieldHom {A : Type ℓ} {B : Type ℓ'} (R : FieldStr A) (f : A → B) (S : FieldStr B)
Algebra\Field\Base.agda:143:  : Type (ℓ-max ℓ ℓ')
Algebra\Field\Base.agda:160:FieldHom : (R : Field ℓ) (S : Field ℓ') → Type (ℓ-max ℓ ℓ')
Algebra\Field\Base.agda:164:IsFieldEquiv : {A : Type ℓ} {B : Type ℓ'}
Algebra\Field\Base.agda:165:  (R : FieldStr A) (e : A ≃ B) (S : FieldStr B) → Type (ℓ-max ℓ ℓ')
Algebra\Field\Base.agda:168:FieldEquiv : (R : Field ℓ) (S : Field ℓ') → Type (ℓ-max ℓ ℓ')
Algebra\Field\Base.agda:180:isPropIsField : {R : Type ℓ} (0r 1r : R) (_+_ _·_ : R → R → R) (-_ : R → R)
Algebra\GradedRing\Base.agda:27:                    (G    : (k : (fst IdM)) → Type ℓ')
Algebra\GradedRing\Base.agda:31:                    : Type (ℓ-max ℓ ℓ')
Algebra\GradedRing\Base.agda:55:record GradedRingStr (R : Ring (ℓ-max ℓ ℓ')) : Type (ℓ-suc (ℓ-max ℓ ℓ')) where
Algebra\GradedRing\Base.agda:61:    G    : (k : (fst IdM)) → Type ℓ'
Algebra\GradedRing\Base.agda:70:GradedRing : ∀ ℓ ℓ' → Type (ℓ-suc (ℓ-max ℓ ℓ'))
Algebra\GradedRing\Base.agda:80:                   {G       : (k : (fst IdM)) → Type ℓ'}
Algebra\GradedRing\Base.agda:104:                 (G      : (k : (fst IdM)) → Type ℓ')
Algebra\GradedRing\Base.agda:134:                     (G      : (k : (fst IdM)) → Type ℓ')
Algebra\GradedRing\DirectSumFun.agda:62:  (G : ℕ → Type ℓ)
Algebra\GradedRing\DirectSumHIT.agda:24:  (G : (n : Idx) → Type ℓ')
Algebra\GradedRing\Instances\CohomologyRing.agda:31:  (A : Type ℓ)
Algebra\GradedRing\Instances\CohomologyRingFun.agda:29:module _ (A : Type ℓ) where
Algebra\GradedRing\Instances\TrivialGradedRing.agda:37:    G : ℕ → Type ℓ
Algebra\Group\Base.agda:22:record IsGroup {G : Type ℓ}
Algebra\Group\Base.agda:23:               (1g : G) (_·_ : G → G → G) (inv : G → G) : Type ℓ where
Algebra\Group\Base.agda:36:record GroupStr (G : Type ℓ) : Type ℓ where
Algebra\Group\Base.agda:52:Group : ∀ ℓ → Type (ℓ-suc ℓ)
Algebra\Group\Base.agda:53:Group ℓ = TypeWithStr ℓ GroupStr
Algebra\Group\Base.agda:55:Group₀ : Type₁
Algebra\Group\Base.agda:58:group : (G : Type ℓ) (1g : G) (_·_ : G → G → G) (inv : G → G) (h : IsGroup 1g _·_ inv) → Group ℓ
Algebra\Group\Base.agda:61:makeIsGroup : {G : Type ℓ} {e : G} {_·_ : G → G → G} { inv : G → G}
Algebra\Group\Base.agda:73:makeGroup : {G : Type ℓ} (1g : G) (_·_ : G → G → G) (inv : G → G)
Algebra\Group\Base.agda:92:makeGroup-right : {A : Type ℓ}
Algebra\Group\Base.agda:137:makeGroup-left : {A : Type ℓ}
Algebra\Group\Exact.agda:35:                 : Type (ℓ-max ℓ (ℓ-max ℓ' ℓ'')) where
Algebra\Group\Exact.agda:41:IsExact : {A B C : Group ℓ} (f : GroupHom A B) (g : GroupHom B C) → Type ℓ
Algebra\Group\Exact.agda:87:  : Type (ℓ-max ℓ (ℓ-max ℓ' (ℓ-max ℓ'' ℓ'''))) where
Algebra\Group\Exact.agda:148:  J4 : ∀ {ℓ ℓ₂ ℓ₃ ℓ₄ ℓ'} {A : Type ℓ}
Algebra\Group\Exact.agda:149:       {A₂ : Type ℓ₂} {A₃ : Type ℓ₃} {A₄ : Type ℓ₄}
Algebra\Group\Exact.agda:152:       → x ≡ y → x₂ ≡ z → x₃ ≡ w → x₄ ≡ u → Type ℓ')
Algebra\Group\Free.agda:18:In the Discrete module, the presence of decidable equality on the type of generators (A) enables the
definition of groups without requiring truncation. This utility is used in `Cubical.HITs.Bouquet.Discrete` to demonstrate that a
bouquet over a discrete type is an hGroupoid without truncation.
Algebra\Group\Free.agda:54:[𝟚×_] : (A : Type ℓ) → Type ℓ
Algebra\Group\Free.agda:57:module NormalForm (A : Type ℓ) where
Algebra\Group\Free.agda:68: IsRedex : Bool × A → Bool × A → Type ℓ
Algebra\Group\Free.agda:74: WillReduce : Bool → A → (xs : [𝟚× A ]) → Type ℓ
Algebra\Group\Free.agda:78: HeadIsRedex : [𝟚× A ] → Type ℓ
Algebra\Group\Free.agda:82: HasRedex : [𝟚× A ] → Type ℓ
Algebra\Group\Free.agda:101: IsNormalised : [𝟚× A ] → Type ℓ
Algebra\Group\Free.agda:173: data _⇊1g : [𝟚× A ] → Type ℓ where
Algebra\Group\Free.agda:288: record _·_⁻¹≡ε (xs ys : _) : Type ℓ where
Algebra\Group\Free.agda:331: List/↘↙ : Type ℓ
Algebra\Group\Free.agda:474:module NF {ℓ'} {A : Type ℓ} (G : Group ℓ') (η : A → ⟨ G ⟩) where
Algebra\Group\Free.agda:485: record NF (g : _) : Type (ℓ-max ℓ ℓ') where
Algebra\Group\GroupPath.agda:58:-- between a group G and a type A which preserves the full raw group
Algebra\Group\GroupPath.agda:60:-- some type equivalent to a group is a group while also specifying
Algebra\Group\GroupPath.agda:62:module _ (G : Group ℓ) {A : Type ℓ}
Algebra\Group\GroupPath.agda:75:    BaseΣ : Type (ℓ-suc ℓ)
Algebra\Group\GroupPath.agda:76:    BaseΣ = Σ[ B ∈ Type ℓ ] (B → B → B) × B × (B → B)
Algebra\Group\GroupPath.agda:78:    FamilyΣ : BaseΣ → Type ℓ
Algebra\Group\GroupPath.agda:104:-- version is useful when proving that some type equivalent to a group
Algebra\Group\GroupPath.agda:108:module _ (G : Group ℓ) {A : Type ℓ}
Algebra\Group\GroupPath.agda:117:    FamilyΣ : Σ[ B ∈ Type ℓ ] (B → B → B) → Type ℓ
Algebra\Group\GroupPath.agda:123:        (UARel.≅→≡ (autoUARel (Σ[ B ∈ Type ℓ ] (B → B → B))) (e , p·))
Algebra\Group\GroupPath.agda:161:GroupEquivJ : {G : Group ℓ} (P : (H : Group ℓ) → GroupEquiv G H → Type ℓ')
Algebra\Group\GroupPath.agda:167:   {P : (H : Group ℓ) → GroupEquiv G H → Type ℓ'} →
Algebra\Group\GroupPath.agda:177:module _ {ℓ ℓ'} {A : Type ℓ}
Algebra\Group\MorphismProperties.agda:41:module _ {A : Type ℓ} {B : Type ℓ'} (G : GroupStr A) (f : A → B) (H : GroupStr B)
Algebra\Group\MorphismProperties.agda:70:module _ {A : Type ℓ} {B : Type ℓ'} {G : GroupStr A} {f : A → B} {H : GroupStr B}
Algebra\Group\Morphisms.agda:34:record IsGroupHom {A : Type ℓ} {B : Type ℓ'}
Algebra\Group\Morphisms.agda:36:  : Type (ℓ-max ℓ ℓ')
Algebra\Group\Morphisms.agda:51:GroupHom : (G : Group ℓ) (H : Group ℓ') → Type (ℓ-max ℓ ℓ')
Algebra\Group\Morphisms.agda:54:GroupIso : (G : Group ℓ) (H : Group ℓ') → Type (ℓ-max ℓ ℓ')
Algebra\Group\Morphisms.agda:57:IsGroupEquiv : {A : Type ℓ} {B : Type ℓ'}
Algebra\Group\Morphisms.agda:58:  (M : GroupStr A) (e : A ≃ B) (N : GroupStr B) → Type (ℓ-max ℓ ℓ')
Algebra\Group\Morphisms.agda:61:GroupEquiv : (G : Group ℓ) (H : Group ℓ') → Type (ℓ-max ℓ ℓ')
Algebra\Group\Morphisms.agda:76:isInIm : GroupHom G H → ⟨ H ⟩ → Type _
Algebra\Group\Morphisms.agda:79:isInKer : GroupHom G H → ⟨ G ⟩ → Type _
Algebra\Group\Morphisms.agda:82:Ker : GroupHom G H → Type _
Algebra\Group\Morphisms.agda:85:Im : GroupHom G H → Type _
Algebra\Group\Morphisms.agda:88:isSurjective : GroupHom G H → Type _
Algebra\Group\Morphisms.agda:91:isInjective : GroupHom G H → Type _
Algebra\Group\Morphisms.agda:94:isMono : GroupHom G H → Type _
Algebra\Group\Morphisms.agda:98:record BijectionIso (G : Group ℓ) (H : Group ℓ') : Type (ℓ-max ℓ ℓ') where
Algebra\Group\Properties.agda:19:    G : Type ℓ
Algebra\Group\Properties.agda:31:isSetGroupStr : (G : Type ℓ) → isSet (GroupStr G)
Algebra\Group\QuotientGroup.agda:40:  _~_ : G → G → Type ℓ
Algebra\Group\QuotientGroup.agda:46:  G/H : Type ℓ
Algebra\Group\QuotientGroup.agda:193:   -- Faster type checking this way...
Algebra\Group\Subgroup.agda:35:  record isSubgroup (H : ℙ G) : Type ℓ where
Algebra\Group\Subgroup.agda:43:  Subgroup : Type (ℓ-suc ℓ)
Algebra\Group\Subgroup.agda:92:  isNormal : Subgroup G' → Type ℓ
Algebra\Group\Subgroup.agda:144:NormalSubgroup : (G : Group ℓ) → Type _
Algebra\Group\ZAction.agda:195:gen₁-by : (G : Group ℓ) → (g : fst G) → Type _
Algebra\Group\ZAction.agda:199:gen₂-by : ∀ {ℓ} (G : Group ℓ) → (g₁ g₂ : fst G) → Type _
Algebra\Group\ZAction.agda:645:-- for type checking reasons, let's also do it with an abstract type
Algebra\Group\Abelianization\AbelianizationAsCoeq.agda:64:  Abelianization : Type ℓ
Algebra\Group\Abelianization\Base.agda:28:    The definition of the abelianization of a group as a higher inductive type.
Algebra\Group\Abelianization\Base.agda:31:  data Abelianization : Type ℓ where
Algebra\Group\Abelianization\Properties.agda:44:  elimProp : {B : Abelianization G → Type ℓ'}
Algebra\Group\Abelianization\Properties.agda:58:  elimProp2 : {C : Abelianization G → Abelianization G → Type ℓ'}
Algebra\Group\Abelianization\Properties.agda:66:  elimProp3 : {D : Abelianization G → Abelianization G → Abelianization G → Type ℓ'}
Algebra\Group\Abelianization\Properties.agda:74:  elimContr : {B : Abelianization G → Type ℓ'}
Algebra\Group\Abelianization\Properties.agda:81:  elimContr2 : {C : Abelianization G → Abelianization G → Type ℓ'}
Algebra\Group\Abelianization\Properties.agda:88:  rec : {M : Type ℓ'}
Algebra\Group\Abelianization\Properties.agda:99:  rec2 : {M : Type ℓ'}
Algebra\Group\Instances\Bool.agda:51:-- Proof that any Group equivalent to Bool as types is also isomorphic to Bool as groups.
Algebra\Group\Instances\IntMod.agda:200:ℤ/2-elim : ∀ {ℓ} {A : Fin 2 → Type ℓ} → A 0 → A 1 → (x : _) → A x
Algebra\Group\Instances\IntMod.agda:205:ℤ/2-rec : ∀ {ℓ} {A : Type ℓ} → A → A → Fin 2 → A
Algebra\Group\Instances\NProd.agda:15:NProd-Group : (G : (n : ℕ) → Type ℓ) → (Gstr : (n : ℕ) → GroupStr (G n)) → Group ℓ
Algebra\Group\Instances\Pi.agda:17:module _ {ℓ ℓ' : Level} {X : Type ℓ} (G : X → Group ℓ') where
Algebra\Group\Instances\Pi.agda:36:ΠGroupHom : ∀ {ℓ ℓ' ℓ''} {A : Type ℓ} {G : A → Group ℓ'} {H : A → Group ℓ''}
Algebra\Group\Instances\Pi.agda:44:ΠGroupIso : ∀ {ℓ ℓ' ℓ''} {A : Type ℓ} {G : A → Group ℓ'} {H : A → Group ℓ''}
Algebra\IntegerMatrix\Base.agda:41:Norm : ℤ → Type
Algebra\IntegerMatrix\Base.agda:84:data PivotOrNot (a : ℤ)(M : Mat (suc m) (suc n)) : Type where
Algebra\IntegerMatrix\Base.agda:98:data NonZeroOrNot (M : Mat (suc m) (suc n)) : Type where
Algebra\IntegerMatrix\Diagonalization.agda:63:isNonZero : List ℤ → Type
Algebra\IntegerMatrix\Diagonalization.agda:71:NonZeroList : Type
Algebra\IntegerMatrix\Diagonalization.agda:97:record isDiagonal (M : Mat m n) : Type where
Algebra\IntegerMatrix\Diagonalization.agda:140:data DivStatus (a : ℤ)(M : Mat (suc m) (suc n)) : Type where
Algebra\IntegerMatrix\Diagonalization.agda:159:record DiagStep (M : Mat (suc m) (suc n)) : Type where
Algebra\IntegerMatrix\Diagonalization.agda:220:record Diag (M : Mat m n) : Type where
Algebra\IntegerMatrix\Elementaries.agda:206:  record RowsImproved (M : Mat (suc m) (suc n)) : Type where
Algebra\IntegerMatrix\Elementaries.agda:214:  record ColsImproved (M : Mat (suc m) (suc n)) : Type where
Algebra\IntegerMatrix\Smith\NormalForm.agda:39:_∣all_ : ℤ → List ℤ → Type
Algebra\IntegerMatrix\Smith\NormalForm.agda:47:isConsDivs : List ℤ → Type
Algebra\IntegerMatrix\Smith\NormalForm.agda:55:ConsDivs : Type
Algebra\IntegerMatrix\Smith\NormalForm.agda:89:record isSmithNormal (M : Mat m n) : Type where
Algebra\IntegerMatrix\Smith\NormalForm.agda:123:record Smith (M : Mat m n) : Type where
Algebra\IntegerMatrix\Smith\Normalization.agda:59:record RowsImprovedTrick (M : Mat (suc m) (suc n)) : Type where
Algebra\IntegerMatrix\Smith\Normalization.agda:86:record PivotReduced (M : Mat (suc m) (suc n)) : Type where
Algebra\IntegerMatrix\Smith\Normalization.agda:102:record RowsImprovedTrickHelper (M : Mat (suc m) (suc n)) : Type where
Algebra\IntegerMatrix\Smith\Normalization.agda:111:record InductionHelper (M : Mat (suc m) (suc n)) : Type where
Algebra\IntegerMatrix\Smith\Normalization.agda:198:record SmithStep (M : Mat (suc m) (suc n)) : Type where
Algebra\Lattice\Base.agda:34:record IsLattice {L : Type ℓ}
Algebra\Lattice\Base.agda:35:                 (0l 1l : L) (_∨l_ _∧l_ : L → L → L) : Type ℓ where
Algebra\Lattice\Base.agda:75:record LatticeStr (A : Type ℓ)  : Type (ℓ-suc ℓ) where
Algebra\Lattice\Base.agda:92:Lattice : ∀ ℓ → Type (ℓ-suc ℓ)
Algebra\Lattice\Base.agda:93:Lattice ℓ = TypeWithStr ℓ LatticeStr
Algebra\Lattice\Base.agda:95:makeIsLattice : {L : Type ℓ} {0l 1l : L} {_∨l_ _∧l_ : L → L → L}
Algebra\Lattice\Base.agda:119:makeLattice : {L : Type ℓ} (0l 1l : L) (_∨l_ _∧l_ : L → L → L)
Algebra\Lattice\Base.agda:138:record IsLatticeHom {A : Type ℓ} {B : Type ℓ'} (L : LatticeStr A) (f : A → B) (M : LatticeStr B)
Algebra\Lattice\Base.agda:139:  : Type (ℓ-max ℓ ℓ')
Algebra\Lattice\Base.agda:156:LatticeHom : (L : Lattice ℓ) (M : Lattice ℓ') → Type (ℓ-max ℓ ℓ')
Algebra\Lattice\Base.agda:166:IsLatticeEquiv : {A : Type ℓ} {B : Type ℓ'} (M : LatticeStr A) (e : A ≃ B) (N : LatticeStr B)
Algebra\Lattice\Base.agda:167:  → Type (ℓ-max ℓ ℓ')
Algebra\Lattice\Base.agda:170:LatticeEquiv : (L : Lattice ℓ) (M : Lattice ℓ') → Type (ℓ-max ℓ ℓ')
Algebra\Lattice\Base.agda:173:isPropIsLattice : {L : Type ℓ} (0l 1l : L) (_∨l_ _∧l_ : L → L → L)
Algebra\Lattice\Base.agda:185:isPropIsLatticeHom : {A : Type ℓ} {B : Type ℓ'} (R : LatticeStr A) (f : A → B) (S : LatticeStr B)
Algebra\Matrix\CommRingCoefficient.agda:47:  Mat : (m n : ℕ) → Type ℓ
Algebra\Matrix\CommRingCoefficient.agda:100:  isInv' : Mat n n → Mat n n → Type ℓ
Algebra\Matrix\CommRingCoefficient.agda:114:  isInv : Mat n n → Type ℓ
Algebra\Matrix\CommRingCoefficient.agda:135:  InvMat : (n : ℕ) → Type ℓ
Algebra\Matrix\CommRingCoefficient.agda:217:  record SimRel (M N : Mat m n) : Type ℓ where
Algebra\Matrix\CommRingCoefficient.agda:228:  record Sim (M : Mat m n) : Type ℓ where
Algebra\Matrix\Elementaries.agda:30:    A : Type ℓ
Algebra\Matrix\Elementaries.agda:154:  record SwapFirstRow (i : Fin m)(M : Mat (suc m) (suc n)) : Type ℓ where
Algebra\Matrix\Elementaries.agda:159:  record SwapFirstCol (j : Fin n)(M : Mat (suc m) (suc n)) : Type ℓ where
Algebra\Matrix\Elementaries.agda:164:  record SwapPivot (i : Fin (suc m))(j : Fin (suc n))(M : Mat (suc m) (suc n)) : Type ℓ where
Algebra\Matrix\Elementaries.agda:305:  record AddFirstRow (M : Mat (suc m) (suc n)) : Type ℓ where
Algebra\Matrix\RowTransformation.agda:32:    A : Type ℓ
Algebra\Matrix\RowTransformation.agda:116:    (P : FinVec A n → Type ℓ)
Algebra\Matrix\RowTransformation.agda:124:    (P : FinVec A n → Type ℓ)
Algebra\Matrix\RowTransformation.agda:132:    (Q : FinVec A n → Type ℓ)
Algebra\Matrix\RowTransformation.agda:133:    (P : FinVec A n → Type ℓ)
Algebra\Matrix\RowTransformation.agda:142:    (Q : FinVec A n → Type ℓ)
Algebra\Matrix\RowTransformation.agda:143:    (P : FinVec A n → Type ℓ)
Algebra\Matrix\RowTransformation.agda:152:    (Rel : FinVec A n → FinVec A n → Type ℓ)
Algebra\Matrix\RowTransformation.agda:160:    (Rel3 : FinVec A n → FinVec A n → FinVec A n → Type ℓ)
Algebra\Matrix\RowTransformation.agda:171:    (Rel : FinVec A n → FinVec A n → Type ℓ)
Algebra\Matrix\RowTransformation.agda:185:    (P : FinVec A n → Type ℓ)
Algebra\Matrix\RowTransformation.agda:186:    (Rel : FinVec A n → FinVec A n → Type ℓ)
Algebra\Matrix\RowTransformation.agda:202:    (P : FinVec A n → Type ℓ)
Algebra\Matrix\RowTransformation.agda:243:  record isLinear2×2 (T : Mat 2 n → Mat 2 n) : Type ℓ where
Algebra\Matrix\RowTransformation.agda:248:  record isLinear (T : Mat m n → Mat k n) : Type ℓ where
Algebra\Matrix\RowTransformation.agda:294:      (P : FinVec R (suc n) → Type ℓ)
Algebra\Matrix\RowTransformation.agda:541:      (P : FinVec R (suc n) → Type ℓ)
Algebra\Module\Base.agda:29:record IsLeftModule (R : Ring ℓ) {M : Type ℓ'}
Algebra\Module\Base.agda:33:  (_⋆_ : ⟨ R ⟩ → M → M) : Type (ℓ-max ℓ ℓ') where
Algebra\Module\Base.agda:53:record LeftModuleStr (R : Ring ℓ) (A : Type ℓ') : Type (ℓ-max ℓ ℓ') where
Algebra\Module\Base.agda:68:LeftModule : (R : Ring ℓ) → ∀ ℓ' → Type (ℓ-max ℓ (ℓ-suc ℓ'))
Algebra\Module\Base.agda:69:LeftModule R ℓ' = Σ[ A ∈ Type ℓ' ] LeftModuleStr R A
Algebra\Module\Base.agda:87:  module _  {M : Type ℓ'} {0m : M}
Algebra\Module\Base.agda:107:record IsLeftModuleHom {R : Ring ℓ} {A B : Type ℓ'}
Algebra\Module\Base.agda:109:  : Type (ℓ-max ℓ ℓ')
Algebra\Module\Base.agda:123:LeftModuleHom : {R : Ring ℓ} (M N : LeftModule R ℓ') → Type (ℓ-max ℓ ℓ')
Algebra\Module\Base.agda:126:IsLeftModuleEquiv : {R : Ring ℓ} {A B : Type ℓ'}
Algebra\Module\Base.agda:128:  → Type (ℓ-max ℓ ℓ')
Algebra\Module\Base.agda:131:LeftModuleEquiv : {R : Ring ℓ} (M N : LeftModule R ℓ') → Type (ℓ-max ℓ ℓ')
Algebra\Module\Base.agda:134:isPropIsLeftModule : (R : Ring ℓ) {M : Type ℓ'}
Algebra\Module\Submodule.agda:23:  record isSubmodule (N : ℙ ⟨ M ⟩) : Type (ℓ-max ℓ ℓ') where
Algebra\Module\Submodule.agda:36:  Submodule : Type _
Algebra\Monoid\Base.agda:26:record IsMonoid {A : Type ℓ} (ε : A) (_·_ : A → A → A) : Type ℓ where
Algebra\Monoid\Base.agda:38:record MonoidStr (A : Type ℓ) : Type ℓ where
Algebra\Monoid\Base.agda:50:Monoid : ∀ ℓ → Type (ℓ-suc ℓ)
Algebra\Monoid\Base.agda:51:Monoid ℓ = TypeWithStr ℓ MonoidStr
Algebra\Monoid\Base.agda:53:monoid : (A : Type ℓ) (ε : A) (_·_ : A → A → A) (h : IsMonoid ε _·_) → Monoid ℓ
Algebra\Monoid\Base.agda:58:makeIsMonoid : {M : Type ℓ} {ε : M} {_·_ : M → M → M}
Algebra\Monoid\Base.agda:68:makeMonoid : {M : Type ℓ} (ε : M) (_·_ : M → M → M)
Algebra\Monoid\Base.agda:77:record IsMonoidHom {A : Type ℓ} {B : Type ℓ'}
Algebra\Monoid\Base.agda:79:  : Type (ℓ-max ℓ ℓ')
Algebra\Monoid\Base.agda:93:MonoidHom : (L : Monoid ℓ) (M : Monoid ℓ') → Type (ℓ-max ℓ ℓ')
Algebra\Monoid\Base.agda:96:IsMonoidEquiv : {A : Type ℓ} {B : Type ℓ'} (M : MonoidStr A) (e : A ≃ B) (N : MonoidStr B)
Algebra\Monoid\Base.agda:97:  → Type (ℓ-max ℓ ℓ')
Algebra\Monoid\Base.agda:100:MonoidEquiv : (M : Monoid ℓ) (N : Monoid ℓ') → Type (ℓ-max ℓ ℓ')
Algebra\Monoid\Base.agda:105:isPropIsMonoid : {M : Type ℓ} (ε : M) (_·_ : M → M → M) → isProp (IsMonoid ε _·_)
Algebra\Monoid\Base.agda:144:  hasInverse : (x : ⟨ M ⟩) → Type ℓ
Algebra\Monoid\Submonoid.agda:14:record isSubmonoid (S : ℙ ⟨ M ⟩) : Type ℓ where
Algebra\Monoid\Submonoid.agda:19:Submonoid : Type (ℓ-suc ℓ)
Algebra\OrderedCommMonoid\Base.agda:7:open import Cubical.Foundations.SIP using (TypeWithStr)
Algebra\OrderedCommMonoid\Base.agda:19:       {M : Type ℓ}
Algebra\OrderedCommMonoid\Base.agda:20:       (_·_ : M → M → M) (1m : M) (_≤_ : M → M → Type ℓ') : Type (ℓ-max ℓ ℓ')
Algebra\OrderedCommMonoid\Base.agda:32:record OrderedCommMonoidStr (ℓ' : Level) (M : Type ℓ) : Type (ℓ-suc (ℓ-max ℓ ℓ')) where
Algebra\OrderedCommMonoid\Base.agda:34:    _≤_ : M → M → Type ℓ'
Algebra\OrderedCommMonoid\Base.agda:43:OrderedCommMonoid : (ℓ ℓ' : Level) → Type (ℓ-suc (ℓ-max ℓ ℓ'))
Algebra\OrderedCommMonoid\Base.agda:44:OrderedCommMonoid ℓ ℓ' = TypeWithStr ℓ (OrderedCommMonoidStr ℓ')
Algebra\OrderedCommMonoid\Base.agda:47:    {M : Type ℓ} {1m : M} {_·_ : M → M → M} {_≤_ : M → M → Type ℓ'}
Algebra\OrderedCommMonoid\Base.agda:69:    {M : Type ℓ} {1m : M} {_·_ : M → M → M} {_≤_ : M → M → Type ℓ'}
Algebra\OrderedCommMonoid\PropCompletion.agda:42:  isUpwardClosed : (s : fst M → hProp ℓ) → Type _
Algebra\OrderedCommMonoid\PropCompletion.agda:52:  M↑ : Type _
Algebra\OrderedCommMonoid\PropCompletion.agda:58:  _isUpperBoundOf_ : fst M → M↑ → Type ℓ
Algebra\OrderedCommMonoid\PropCompletion.agda:61:  isBounded : (s : M↑) → Type _
Algebra\OrderedCommMonoid\PropCompletion.agda:100:  typeAt : fst M → M↑ → Type _
Algebra\OrderedCommMonoid\PropCompletion.agda:101:  typeAt n s = fst (fst s n)
Algebra\OrderedCommMonoid\PropCompletion.agda:103:  M↑Path : {s l : M↑} → ((n : fst M) → typeAt n s ≡ typeAt n l) → s ≡ l
Algebra\OrderedCommMonoid\PropCompletion.agda:114:           → ((n : fst M) → typeAt n s → typeAt n l)
Algebra\OrderedCommMonoid\PropCompletion.agda:115:           → ((n : fst M) → typeAt n l → typeAt n s)
Algebra\OrderedCommMonoid\PropCompletion.agda:127:      ⇐ : (n : fst M) → typeAt n ((x · y) ^↑) → typeAt n ((x ^↑) ·↑ (y ^↑))
Algebra\OrderedCommMonoid\PropCompletion.agda:130:      ⇒ : (n : fst M) → typeAt n ((x ^↑) ·↑ (y ^↑)) → typeAt n ((x · y) ^↑)
Algebra\OrderedCommMonoid\PropCompletion.agda:153:    where ⇒ : (n : fst M) → typeAt n (s ·↑ 1↑) → typeAt n s
Algebra\OrderedCommMonoid\PropCompletion.agda:158:          ⇐ : (n : fst M) → typeAt n s → typeAt n (s ·↑ 1↑)
Algebra\OrderedCommMonoid\PropCompletion.agda:163:    where ⇒ : (n : fst M) → typeAt n (s ·↑ (l ·↑ k)) → typeAt n ((s ·↑ l) ·↑ k)
Algebra\OrderedCommMonoid\PropCompletion.agda:194:  _≤↑_ : (s l : M↑) → Type _
Algebra\OrderedCommMonoid\Properties.agda:6:open import Cubical.Foundations.SIP using (TypeWithStr)
Algebra\OrderedCommMonoid\Properties.agda:31:      subtype = Σ[ x ∈ ⟨ M ⟩ ] ⟨ P x ⟩
Algebra\OrderedCommMonoid\Properties.agda:33:      _≤ₛ_ : (x y : ⟨ submonoid ⟩) → Type ℓ'
Algebra\OrderedCommMonoid\Properties.agda:39:    fst makeOrderedSubmonoid = subtype
Algebra\OrderedCommRing\Base.agda:29:  {R : Type ℓ}
Algebra\OrderedCommRing\Base.agda:33:  (_<_ _≤_ : R → R → Type ℓ') : Type (ℓ-max ℓ ℓ') where
Algebra\OrderedCommRing\Base.agda:56:record OrderedCommRingStr (ℓ' : Level) (R : Type ℓ) : Type (ℓ-suc (ℓ-max ℓ ℓ')) where
Algebra\OrderedCommRing\Base.agda:62:    _<_ _≤_ : R → R → Type ℓ'
Algebra\OrderedCommRing\Base.agda:72:OrderedCommRing : (ℓ ℓ' : Level) → Type (ℓ-suc (ℓ-max ℓ ℓ'))
Algebra\OrderedCommRing\Base.agda:73:OrderedCommRing ℓ ℓ' = TypeWithStr ℓ (OrderedCommRingStr ℓ')
Algebra\OrderedCommRing\Base.agda:75:module _ {R : Type ℓ} {0r 1r : R} {_+_ _·_ : R → R → R} { -_ : R → R }
Algebra\OrderedCommRing\Base.agda:76:  {_<_ _≤_ : R → R → Type ℓ'}
Algebra\Polynomials\TypevariateHIT.agda:1:module Cubical.Algebra.Polynomials.TypevariateHIT where
Algebra\Polynomials\TypevariateHIT.agda:3:open import Cubical.Algebra.Polynomials.TypevariateHIT.Base public
Algebra\Polynomials\Multivariate\EquivCarac\A[X]X-A.agda:46:    A[x] : Type ℓ
Algebra\Polynomials\Multivariate\EquivCarac\A[X]X-A.agda:52:    A[x]/x : Type ℓ
Algebra\Polynomials\TypevariateHIT\Base.agda:1:module Cubical.Algebra.Polynomials.TypevariateHIT.Base where
Algebra\Polynomials\TypevariateHIT\Base.agda:4:Typevariate polynomials over a commutative ring
Algebra\Polynomials\TypevariateHIT\Base.agda:7:For a univariate polynomial, the type of variables is the unit type, for multivariate
Algebra\Polynomials\TypevariateHIT\Base.agda:8:polynomials it is a standard finite type. For a typevariate polynomial, the type of
variables
Algebra\Polynomials\TypevariateHIT\Base.agda:9:is an arbitrary type I : Type. Since the CommRing R[I] is 0-truncated, the polynomials
only depend
Algebra\Polynomials\TypevariateHIT\Base.agda:11:The typevariate polynomials are constructed as a free commutative algebra on the given
I : Type.
Algebra\Polynomials\TypevariateHIT\EquivUnivariateListPoly.agda:4:module
Cubical.Algebra.Polynomials.TypevariateHIT.EquivUnivariateListPoly where
Algebra\Polynomials\TypevariateHIT\EquivUnivariateListPoly.agda:65:  typevariateListPolyIso : Iso ⟨ R [ Unit ] ⟩  ⟨ ListPolyCommAlgebra
R ⟩
Algebra\Polynomials\TypevariateHIT\EquivUnivariateListPoly.agda:66:  fun typevariateListPolyIso = fst to
Algebra\Polynomials\TypevariateHIT\EquivUnivariateListPoly.agda:67:  inv typevariateListPolyIso = fst from
Algebra\Polynomials\TypevariateHIT\EquivUnivariateListPoly.agda:68:  sec typevariateListPolyIso = toFrom
Algebra\Polynomials\TypevariateHIT\EquivUnivariateListPoly.agda:69:  ret typevariateListPolyIso = fromTo
Algebra\Polynomials\TypevariateHIT\EquivUnivariateListPoly.agda:71:  typevariateListPolyEquiv : CommAlgebraEquiv (R [ Unit ])
(ListPolyCommAlgebra R)
Algebra\Polynomials\TypevariateHIT\EquivUnivariateListPoly.agda:72:  fst typevariateListPolyEquiv = isoToEquiv typevariateListPolyIso
Algebra\Polynomials\TypevariateHIT\EquivUnivariateListPoly.agda:73:  snd typevariateListPolyEquiv = snd to
Algebra\Polynomials\TypevariateHIT\EquivUnivariateListPoly.agda:75:  typevariateListPolyGenerator :
Algebra\Polynomials\TypevariateHIT\EquivUnivariateListPoly.agda:76:    fst (fst typevariateListPolyEquiv) X-HIT ≡ X-List
Algebra\Polynomials\TypevariateHIT\EquivUnivariateListPoly.agda:77:  typevariateListPolyGenerator = refl
Algebra\Polynomials\UnivariateFun\Base.agda:8:The base type is define using almost null sequences ie functions.
Algebra\Polynomials\UnivariateFun\Base.agda:10:Thus base type and the AbGroup part of the CommRing is define an instance
Algebra\Polynomials\UnivariateHIT\Base.agda:8:The base type is define using an HIT.
Algebra\Polynomials\UnivariateHIT\Base.agda:10:Thus base type and the AbGroup part of the CommRing is define an instance
Algebra\Polynomials\UnivariateList\Base.agda:27:    A : Type ℓ
Algebra\Polynomials\UnivariateList\Base.agda:35:  data Poly : Type ℓ where
Algebra\Polynomials\UnivariateList\Base.agda:54:  module Elim (B      : Poly R' → Type ℓ')
Algebra\Polynomials\UnivariateList\Base.agda:64:  -- Given a proposition (as type) ϕ ranging over polynomials, we prove it by:
Algebra\Polynomials\UnivariateList\Base.agda:67:  module _ (B : Poly R' → Type ℓ')
Algebra\Polynomials\UnivariateList\Base.agda:75:  module _ (B         : Poly R' → Poly R' → Type ℓ')
Algebra\Polynomials\UnivariateList\Base.agda:90:  module Rec (B : Type ℓ')
Algebra\Polynomials\UnivariateList\Base.agda:115:  PolyFun : Type ℓ
Algebra\Ring\Base.agda:33:record IsRing {R : Type ℓ}
Algebra\Ring\Base.agda:34:              (0r 1r : R) (_+_ _·_ : R → R → R) (-_ : R → R) : Type ℓ where
Algebra\Ring\Base.agda:62:record RingStr (A : Type ℓ) : Type (ℓ-suc ℓ) where
Algebra\Ring\Base.agda:83:Ring : ∀ ℓ → Type (ℓ-suc ℓ)
Algebra\Ring\Base.agda:84:Ring ℓ = TypeWithStr ℓ RingStr
Algebra\Ring\Base.agda:86:module _ {R : Type ℓ} {0r 1r : R} {_+_ _·_ : R → R → R} { -_ : R → R}
Algebra\Ring\Base.agda:105:module _ {R : Type ℓ} (0r 1r : R) (_+_ _·_ : R → R → R) (-_ : R → R)
Algebra\Ring\Base.agda:129:record IsRingHom {A : Type ℓ} {B : Type ℓ'} (R : RingStr A) (f : A → B) (S : RingStr B)
Algebra\Ring\Base.agda:130:  : Type (ℓ-max ℓ ℓ')
Algebra\Ring\Base.agda:147:RingHom : (R : Ring ℓ) (S : Ring ℓ') → Type (ℓ-max ℓ ℓ')
Algebra\Ring\Base.agda:150:IsRingEquiv : {A : Type ℓ} {B : Type ℓ'} (M : RingStr A) (e : A ≃ B) (N : RingStr B)
Algebra\Ring\Base.agda:151:  → Type (ℓ-max ℓ ℓ')
Algebra\Ring\Base.agda:154:RingEquiv : (R : Ring ℓ) (S : Ring ℓ') → Type (ℓ-max ℓ ℓ')
Algebra\Ring\Base.agda:169:isPropIsRing : {R : Type ℓ} (0r 1r : R) (_+_ _·_ : R → R → R) (-_ : R → R)
Algebra\Ring\Base.agda:180:isPropIsRingHom : {A : Type ℓ} {B : Type ℓ'} (R : RingStr A) (f : A → B) (S : RingStr B)
Algebra\Ring\Base.agda:266:module _ (R : Ring ℓ) {A : Type ℓ}
Algebra\Ring\Base.agda:281:    BaseΣ : Type (ℓ-suc ℓ)
Algebra\Ring\Base.agda:282:    BaseΣ = Σ[ B ∈ Type ℓ ] B × B × (B → B → B) × (B → B → B) × (B → B)
Algebra\Ring\Base.agda:284:    FamilyΣ : BaseΣ → Type ℓ
Algebra\Ring\Properties.agda:268:isSetRingStr : (R : Type ℓ) → isSet (RingStr R)
Algebra\Ring\Properties.agda:302:recPT→Ring : {A : Type ℓ'} (𝓕  : A → Ring ℓ)
Algebra\Ring\Quotient.agda:32:  R/I : Type ℓ
Algebra\Ring\Ideal\Base.agda:23:  record isIdeal (I : ℙ R) : Type ℓ where
Algebra\Ring\Ideal\Base.agda:45:  Ideal : Type _
Algebra\Ring\Ideal\Base.agda:48:  record isLeftIdeal (I : ℙ R) : Type ℓ where
Algebra\Ring\Ideal\Base.agda:56:  record isRightIdeal (I : ℙ R) : Type ℓ where
Algebra\Ring\Ideal\Base.agda:93:IdealsIn : (R : Ring ℓ) → Type _
Algebra\Semigroup\Base.agda:32:record IsSemigroup {A : Type ℓ} (_·_ : A → A → A) : Type ℓ where
Algebra\Semigroup\Base.agda:42:record SemigroupStr (A : Type ℓ) : Type ℓ where
Algebra\Semigroup\Base.agda:54:Semigroup : ∀ ℓ → Type (ℓ-suc ℓ)
Algebra\Semigroup\Base.agda:55:Semigroup ℓ = TypeWithStr ℓ SemigroupStr
Algebra\Semigroup\Base.agda:57:module _ (A : Type ℓ) (_·_ : A → A → A) (h : IsSemigroup _·_) where
Algebra\Semigroup\Base.agda:63:record IsSemigroupEquiv {A : Type ℓ} {B : Type ℓ}
Algebra\Semigroup\Base.agda:65:  : Type ℓ
Algebra\Semigroup\Base.agda:80:SemigroupEquiv : (M N : Semigroup ℓ) → Type ℓ
Algebra\Semigroup\Base.agda:83:isPropIsSemigroup : {A : Type ℓ} (_·_ : A → A → A) → isProp (IsSemigroup _·_)
Algebra\Semilattice\Base.agda:46:record IsSemilattice {A : Type ℓ} (ε : A) (_·_ : A → A → A) : Type ℓ where
Algebra\Semilattice\Base.agda:57:record SemilatticeStr (A : Type ℓ) : Type ℓ where
Algebra\Semilattice\Base.agda:69:Semilattice : ∀ ℓ → Type (ℓ-suc ℓ)
Algebra\Semilattice\Base.agda:70:Semilattice ℓ = TypeWithStr ℓ SemilatticeStr
Algebra\Semilattice\Base.agda:72:semilattice : (A : Type ℓ) (ε : A) (_·_ : A → A → A) (h : IsSemilattice ε _·_) → Semilattice ℓ
Algebra\Semilattice\Base.agda:76:makeIsSemilattice : {L : Type ℓ} {ε : L} {_·_ : L → L → L}
Algebra\Semilattice\Base.agda:87:makeSemilattice : {L : Type ℓ} (ε : L) (_·_ : L → L → L)
Algebra\Semilattice\Base.agda:98:SemilatticeStr→MonoidStr : {A : Type ℓ} → SemilatticeStr A → MonoidStr A
Algebra\Semilattice\Base.agda:110:SemilatticeHom : (L : Semilattice ℓ) (M : Semilattice ℓ') → Type (ℓ-max ℓ ℓ')
Algebra\Semilattice\Base.agda:113:IsSemilatticeEquiv : {A : Type ℓ} {B : Type ℓ'}
Algebra\Semilattice\Base.agda:114:  (M : SemilatticeStr A) (e : A ≃ B) (N : SemilatticeStr B) → Type (ℓ-max ℓ ℓ')
Algebra\Semilattice\Base.agda:118:SemilatticeEquiv : (M : Semilattice ℓ) (N : Semilattice ℓ') → Type (ℓ-max ℓ ℓ')
Algebra\Semilattice\Base.agda:121:isPropIsSemilattice : {L : Type ℓ} (ε : L) (_·_ : L → L → L)
Algebra\Semilattice\Base.agda:153: _≤_ : L → L → Type ℓ
Algebra\Semilattice\Base.agda:222: _≤_ : L → L → Type ℓ
Algebra\Semiring\Base.agda:4:open import Cubical.Foundations.SIP using (TypeWithStr)
Algebra\Semiring\Base.agda:13:record IsSemiring {R : Type ℓ}
Algebra\Semiring\Base.agda:14:                  (0r 1r : R) (_+_ _·_ : R → R → R) : Type ℓ where
Algebra\Semiring\Base.agda:39:record SemiringStr (A : Type ℓ) : Type (ℓ-suc ℓ) where
Algebra\Semiring\Base.agda:53:Semiring : ∀ ℓ → Type (ℓ-suc ℓ)
Algebra\Semiring\Base.agda:54:Semiring ℓ = TypeWithStr ℓ SemiringStr
Algebra\Semiring\Base.agda:56:makeIsSemiring : {R : Type ℓ} {0r 1r : R} {_+_ _·_ : R → R → R}
Algebra\Semiring\Base.agda:81:  (S : Type ℓ)
AlgebraicGeometry\Functorial\ZFunctors\Base.agda:77:  isAffine : (X : ℤFunctor) → Type (ℓ-suc ℓ)
AlgebraicGeometry\Functorial\ZFunctors\Base.agda:82:  isLocal : ℤFunctor → Type (ℓ-suc ℓ)
AlgebraicGeometry\Functorial\ZFunctors\CompactOpen.agda:131:  CompactOpen : ℤFunctor → Type (ℓ-suc ℓ)
AlgebraicGeometry\Functorial\ZFunctors\CompactOpen.agda:156:  isAffineCompactOpen : {X : ℤFunctor} (U : CompactOpen X) → Type (ℓ-suc ℓ)
AlgebraicGeometry\Functorial\ZFunctors\QcQsScheme.agda:51:  record AffineCover : Type (ℓ-suc ℓ) where
AlgebraicGeometry\Functorial\ZFunctors\QcQsScheme.agda:58:  hasAffineCover : Type (ℓ-suc ℓ)
AlgebraicGeometry\Functorial\ZFunctors\QcQsScheme.agda:64:  isQcQsScheme : ℤFunctor → Type (ℓ-suc ℓ)
AlgebraicGeometry\ZariskiLattice\Base.agda:79:  _≼_ : A → A → Type ℓ
AlgebraicGeometry\ZariskiLattice\Base.agda:94:  _∼_ : A → A → Type ℓ -- \sim
AlgebraicGeometry\ZariskiLattice\Base.agda:111:  ZL : Type ℓ
AlgebraicGeometry\ZariskiLattice\Base.agda:126:  _∼≡_ : A → A → Type (ℓ-suc ℓ)
AlgebraicGeometry\ZariskiLattice\Properties.agda:197:  -- does not type check without lossy unification!!!
AlgebraicGeometry\ZariskiLattice\StructureSheaf.agda:110: BO : Type ℓ
AlgebraicGeometry\ZariskiLattice\StructureSheaf.agda:154:  P : ZL → Type _
AlgebraicGeometry\ZariskiLattice\StructureSheaf.agda:190:     -- but somehow Agda takes forever to type-check if you don't use
-----------------------------
AlgebraicGeometry\ZariskiLattice\StructureSheafPullback.agda:113: BO : Type ℓ
AlgebraicGeometry\ZariskiLattice\StructureSheafPullback.agda:137:  P : ZL → Type _
AlgebraicGeometry\ZariskiLattice\StructureSheafPullback.agda:234:   -- write everything explicitly so things can type-check
AlgebraicGeometry\ZariskiLattice\UniversalProperty.agda:73: record IsSupport (d : R → L) : Type (ℓ-max ℓ ℓ') where
AlgebraicGeometry\ZariskiLattice\UniversalProperty.agda:208:                        → Type _
...
Talks\EPA2020.agda:3:Cubical Agda - A Dependently Typed PL with Univalence and HITs
Talks\EPA2020.agda:32:apply0 : (A : Type) (p : I → A) → A
Talks\EPA2020.agda:40:refl' : {A : Type} (x : A) → x ≡ x
Talks\EPA2020.agda:44:refl'' : {A : Type} (x : A) → PathP (λ _ → A) x x
Talks\EPA2020.agda:47:-- In general PathP A x y has A : I → Type, x : A i0 and y : A i1
Talks\EPA2020.agda:51:-- foo : {A : Type} → I → A
Talks\EPA2020.agda:55:cong' : {A B : Type} (f : A → B) {x y : A} → x ≡ y → f x ≡ f y
Talks\EPA2020.agda:60:funExt' : {A B : Type} {f g : A → B} (p : (x : A) → f x ≡ g x) → f ≡ g
Talks\EPA2020.agda:65:transport' : {A B : Type} → A ≡ B → A → B
Talks\EPA2020.agda:69:subst' : {A : Type} (P : A → Type) {x y : A} (p : x ≡ y) → P x → P y
Talks\EPA2020.agda:72:-- The transp operation reduces differently for different types
Talks\EPA2020.agda:77:J' : {A : Type} {B : A → Type} {x : A}
Talks\EPA2020.agda:78:     (P : (z : A) → x ≡ z → Type)
Talks\EPA2020.agda:92:-- The univalence theorem: equivalences of types give paths of types
Talks\EPA2020.agda:93:ua' : {A B : Type} → A ≃ B → A ≡ B
Talks\EPA2020.agda:96:-- Any isomorphism of types gives rise to an equivalence
Talks\EPA2020.agda:97:isoToEquiv' : {A B : Type} → Iso A B → A ≃ B
Talks\EPA2020.agda:101:isoToPath' : {A B : Type} → Iso A B → A ≡ B
Talks\EPA2020.agda:106:uaβ' : {A B : Type} (e : A ≃ B) (x : A)
Talks\EPA2020.agda:115:data Bool : Type where
Talks\EPA2020.agda:150:-- Higher inductive types
Talks\EPA2020.agda:157:data FMSet (A : Type) : Type where
Talks\EPA2020.agda:166:_++_ : ∀ {A : Type} (xs ys : FMSet A) → FMSet A
Talks\EPA2020.agda:173:unitr-++ : {A : Type} (xs : FMSet A) → xs ++ [] ≡ xs
Talks\EPA2020.agda:183:data _/_ (A : Type) (R : A → A → Type) : Type where
Talks\EPA2020.agda:196:data S¹ : Type where
Talks\EPA2020.agda:205:helix : S¹ → Type
Talks\EPA2020.agda:209:ΩS¹ : Type
Talks\EPA2020.agda:220:data Torus : Type where
Talks\EPA2020.agda:298:-- data AssocList (A : Type) : Type where
Talks\EPA2020.agda:315:substIso : {A B : Type} (P : Type → Type) (e : Iso A B) → P A → P B
Talks\EPA2020.agda:327:sip' : {ℓ : Level} {S : Type ℓ → Type ℓ} {ι : StrEquiv S ℓ}
Talks\EPA2020.agda:328:       (θ : UnivalentStr S ι) (A B : TypeWithStr ℓ S) → A ≃[ ι ] B → A ≡ B
