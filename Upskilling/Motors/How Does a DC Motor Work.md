...
- After the coil has rotated exactly 90 degrees, the (previously) left side of the commutator now touches the right brush and vice versa. This causes the current to be reversed, and the force to be reversed. So the armature keeps spinning. It always tries to put negative to positive and positive to negative in the two magnetic fields, but whenever it gets close the current switches, field s
